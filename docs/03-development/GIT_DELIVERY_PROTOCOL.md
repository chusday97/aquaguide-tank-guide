...
- Before a release reconciliation, run `git fetch origin --prune` followed by `npm run audit:branch-convergence`; use `npm run check:branch-convergence` when local/remote canonical parity must block delivery.
