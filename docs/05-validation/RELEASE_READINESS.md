...
| Exact Preview SHA parity | The read-only gate checks the current candidate head against GitHub deployment metadata; Vercel currently returns `build-rate-limit`, so no Preview deployment exists for the latest head | Pending / rate limited | After the quota window resets, deploy once and require `environment=Preview`, exact SHA and `state=success`. |
