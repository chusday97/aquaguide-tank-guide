...
| --- | --- | --- | --- |
| One delivery line | Local candidate, remote candidate and Draft PR #142 are checked at runtime by `npm run project:status`; current exact head is `7420919c` | Blocked / Supabase and acceptance pending | Complete Supabase parity, then obtain user release acceptance. |
...
| Exact Preview SHA parity | `npm run check:preview-parity` passed for prior code head `55a37745`; the latest docs-only head `7420919c` has an AquaGuide Vercel deployment and needs one final exact-SHA read after checks settle | Pending / recheck | Require `environment=Preview`, exact SHA and `state=success` for `7420919c`. |
