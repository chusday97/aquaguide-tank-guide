...
| --- | --- | --- | --- |
| One delivery line | Local candidate, remote candidate and PR #142 are synchronized at `9bdf2726`; `npm run project:status` passes | Pass / Supabase and acceptance pending | Keep the same SHA through Preview and release acceptance. |
...
| Exact Preview SHA parity | Current candidate `9bdf2726` has not yet been verified against a new exact Preview deployment; earlier Preview evidence is historical | Pending / external Vercel gate | When quota permits, verify `environment=Preview`, exact SHA and `state=success` for `9bdf2726`. |
