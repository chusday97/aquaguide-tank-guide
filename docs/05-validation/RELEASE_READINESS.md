...
| --- | --- | --- | --- |
| One delivery line | Local candidate is `510244d1` and is 5 commits ahead of remote candidate/PR #142 (`4f9e2d71`); `npm run project:status` correctly fails until the planned one-time push | Blocked / local sync, Supabase and acceptance pending | Complete local review, push once, then verify PR SHA parity. |
...
| Exact Preview SHA parity | `npm run check:preview-parity` passed only for an earlier code head; current local head `510244d1` has not been pushed and therefore has no Preview deployment | Pending / not synchronized | After the one-time push and quota reset, require `environment=Preview`, exact SHA and `state=success` for the pushed head. |
