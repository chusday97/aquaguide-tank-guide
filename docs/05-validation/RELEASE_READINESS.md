...
| --- | --- | --- | --- |
| One delivery line | Local candidate, remote candidate and Draft PR #142 are checked at runtime by `npm run project:status`; the latest check passes at the same candidate SHA | Blocked / Supabase and acceptance pending | Complete Supabase parity, then obtain user release acceptance. |
...
| GitHub convergence CI | Foundation, Product Golden Path run `33085634111`, Cloudflare Pages and Vercel checks passed at candidate `02457dd2` | Pass | Any later commit must receive a fresh exact-head run. |
| Exact Preview SHA parity | The final candidate deployment must report the exact SHA from `npm run project:status`; the deployment record is checked externally because documentation commits would otherwise make a self-referential SHA | Pending final candidate deployment | Query the deployment API for the pushed head and record the immutable ID in the release evidence before acceptance. |
