...
| Exact Preview SHA parity | The current candidate head was checked against the GitHub deployment status API; the exact deployment record is kept as external release evidence to avoid self-referential documentation churn | Pass | For every new push, query `deployments?sha=$(git rev-parse HEAD)` and require `environment=Preview`, exact SHA and `state=success`. |
