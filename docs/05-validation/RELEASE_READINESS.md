...
| --- | --- | --- | --- |
| One delivery line | Local candidate is runtime-derived from `git rev-parse HEAD`; remote candidate and Draft PR #142 are still on `5b419e98`; `npm run project:status` fails until the candidate is pushed | Blocked / remote sync pending | Push the reviewed candidate once, then re-run `project:status` and record PR/Preview SHAs. |
...
| GitHub convergence CI | Historical runs `33041753905`, `33041755993` and `33041756115` passed on the old PR head; they do not cover the runtime-derived local HEAD | Pending | Push the candidate and require fresh CI on the exact PR head. |
