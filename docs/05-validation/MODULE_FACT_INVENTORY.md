...
| Compatibility | species detail and compatibility entry points | `src/lib/tankCompatibilityEngine.ts`, `packages/domain-rules/bioload.ts` | species/tank facts; local compatibility input | compatibility 17/17, P0 evidence/state tests | local P0 verified; UI authority remains fallback | `PARTIAL_WITH_FALLBACK` |
...
| Encyclopedia and species detail | `src/pages/Encyclopedia.tsx` | species/compatibility services and visual result adapters | species catalog, optional API content | interactive scenes, split workspace and species detail tests | deployed reverify pending | `CURRENT_VERIFIED` |
