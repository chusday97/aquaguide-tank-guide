...
| Compatibility | species detail, compatibility and recommendation entry points | `src/lib/tankCompatibilityEngine.ts`, `packages/domain-rules/bioload.ts`, `src/modules/knowledge/compatibilityKnowledge.ts`, `src/modules/recommendation/recommendation.service.ts` | species/tank facts; local compatibility input | compatibility 17/17, recommendation authority contract, pair-evidence coverage matrix/scorecard, P0 evidence/state tests | local P0 verified; broad evidence coverage and cloud parity pending | `PARTIAL_WITH_FALLBACK` |
...
| Repository/API/Supabase | repository provider and API services | `src/services/repository/`, `apps/api/`, `api/v1/[...path].ts` | `CONTRACT.md`, database types, RLS | three-tier, API boundary, production-cloud runtime, business API and repository tests | user-confirmed deployed; exact SHA/schema/RLS parity pending | `DEPLOYED_REVERIFY_PENDING` |
| Result UX workflow governance | `.github/workflows/result-ux-head-integrity-v1.yml` | candidate-head checkout and exact SHA assertion | GitHub Actions metadata only; no product/API/data contract | `npm run test:result-ux-head-integrity` | local verified; remote run and release parity pending | `CURRENT_VERIFIED` |
