...
图鉴提供“互动场景”和“传统浏览”两种入口：用户可先在鱼缸画面中点选感兴趣的生物、再查看当前鱼缸的混养预览；传统列表继续承担搜索、筛选和完整资料浏览。养护指南提供“鱼缸场景找问题”：用户先点水面、水体、生物、过滤、底床或设备，再进入确定性的风险分层与下一步。

- 首屏永远先显示现在应做什么，其后才显示原因和完整依据。
- 红旗问题直接展示安全应急动作；普通问题最多补问两项。
- 行动必须回链到现有养护文章、清单、时间线或提醒，禁止停在说明文本。
- 该阶段仅新增前端展示与会话状态，复用现有内容与规则，不新增疾病确诊或开放式 AI 问答。
