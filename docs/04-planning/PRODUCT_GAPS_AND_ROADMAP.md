...
| P0-07 | 页面直写 localStorage 与服务写入并存，可能造成页面不同步 | 鱼缸、巡检、收藏、死亡、养护提醒/操作/清单已进入服务；互动发现牌堆现已通过 `local-app-state` 统一读写并发出变更事件，图鉴筛选使用统计和未路由 AI 聊天仍是页面级 UI 存储 | 业务写入收口服务层并派发统一事件 | 业务状态无页面直写；跨页面即时刷新 | 部分完成 |
