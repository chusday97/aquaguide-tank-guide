...
> 记录日期：2026-07-16。状态基于当前代码与浏览器行为，不以旧概念稿为依据。
...
| P0-07 | 页面直写 localStorage 与服务写入并存，可能造成页面不同步 | 鱼缸、巡检、收藏、死亡、养护提醒/操作/清单已进入服务；互动发现牌堆现已通过 `local-app-state` 统一读写，延迟 patch 在 flush 时合并最新快照并发出变更事件；图鉴筛选使用统计和未路由 AI 聊天仍是页面级 UI 存储 | 业务写入收口服务层并派发统一事件 | 业务状态无页面直写；跨页面即时刷新 | 部分完成 |
...
| P1-10 | 图鉴卡收藏必须先打开详情，路径长且用户不确定收藏去向 | 卡片左上角独立收藏；品类进入变种后逐个收藏；统一同步水族册 | 桌面/手机快捷收藏、刷新保持、失败回滚和水族册同步均通过 | 已完成 |
| P1-11 | 今日行动与养护计划分成两张卡，缸内物种入口重复且目标不可见 | 计划内嵌今日行动；鱼缸画面下只保留一个可展开物种入口 | 一个行动主操作、一个计划摘要、一个正式物种入口 | 已完成 |
| P1-12 | 单页巡检需要手动寻找下一题，连续回答节奏被打断 | 选择后保留反馈并聚焦下一道有效问题；最后聚焦提交 | 鼠标、触控、键盘和减少动态效果均可完成，不自动提交 | 已完成 |
| P1-13 | 语言切换缺少统一入口，英文内容可能与规则输入混用 | 桌面/手机共享语言偏好；展示层本地化，规则继续读取中文主数据 | 首选、切换、持久化、回退和中英文规则一致性通过 | 部分完成：基础框架与设置已完成，内容审核待完成 |
...
| P2-06 | 批量物种图片存在浅色低对比、贴边或主体碎片化 | 18 张候选已逐项复核：16 张主体完整并沿用非破坏性显示保护，2 张确认断裂/碎片化 | 重新生成 `sp_0357` 与 `sp_0452`，候选验收后再替换并重跑素材测试 | 人工复核完成 / 2 张待生成 |
| P2-07 | 三步路径与核心功能仍缺真实水族新手任务证据 | 使用固定任务脚本记录完成率、误点和放弃位置，不预填结果 | 至少一轮真实参与者记录且原始结果可追溯 | 等待外部参与者 |
| P2-08 | 486 个物种与 41 篇文章没有已审核英文版本 | 机器初译只生成草稿，经过术语、数值、安全语义与人工审核后发布 | 核心公开内容已发布覆盖率 100%，未审核内容不可读取 | 待执行 |
| P2-09 | 当前没有真实激活率、TTV、留存或真人任务证据 | 先建立隐私安全的事件和匿名任务记录，再用真实数据验证 H1–H5 | 空数据不生成指标；真实记录可追溯到版本与任务 | 本轮建立基线 |
| P2-10 | AI 场景硬编码在多个脚本，Mock 容易被误认为真实模型质量 | 迁移为版本化 JSONL，分离 deterministic / mocked / live，并建立 Badcase 回流 | 报告明确来源；Live 需显式开启；历史回归可重复运行 | 本轮实施 |
...
建立证据矩阵、结构化 AI 数据集、安全事件和真人测试模板。自动测试只证明实现与规则，不替代真人理解度或真实模型质量。
