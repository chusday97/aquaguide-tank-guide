...
产品包含四个正式核心模块；搜索、拍照识别、设置和首次引导是任务型辅助路由。数据统计、提醒中心不属于当前产品架构；养护计划作为“我的鱼缸”内的紧凑区块展示，不扩展为新的导航中心。

## 2. 导航规则

| 设备布局 | 主导航 | 辅助入口 | 规则 |
|---|---|---|---|
| 桌面、平板 | 左侧栏：我的鱼缸、图鉴、养护指南、我的水族册 | 顶部搜索/拍照，底部设置 | 侧栏选择进入正式路由，不直接打开弹窗；窗口缩窄不切成手机底栏 |
| 真实手机 | 底部栏：我的鱼缸、图鉴、养护指南、我的水族册 | 页头搜索、拍照和设置 | 使用手机专属排版；辅助入口同样进入正式路由 |

设备布局由共享 viewport contract 决定：`<768px` 使用手机结构，`>=768px` 使用桌面/平板工作台；页面内容根据可用容器宽度重排，不由 UA 或设备字符串决定。
...
| `/welcome` | 新用户选择“先建缸 / 先看物种” | 现有本地数据、引导偏好 | 辅助任务 |
| `/search?q=` | 同时搜索物种和养护指南并直达结果 | 双语物种与文章摘要 | 辅助任务 |
...
|---|---|---|---|
| 我的鱼缸 | 当前状态与今天要做的事 | 唯一缸内物种入口、维护记录、3D 展示 | 内容详情仍可使用中央弹窗/手机底部面板；导航不触发弹窗 |
...
- 路由与布局壳：`src/App.tsx`
- 页面：`src/pages/`
- 导航：`src/App.tsx` 中的 `DesktopSidebar`、`MobileAppShell`
