    ...
    <section className={`visual-result-card overflow-hidden rounded-[22px] border bg-white shadow-sm ${className}`} data-visual-result-status={model.status} data-visual-result-presentation={model.presentationMode || 'verdict'}>
      ...
      {model.actionItems && model.actionItems.length > 0 && (
        <section className="mx-3.5 mb-3 rounded-[16px] border border-emerald-100 bg-emerald-50/70 px-3 py-2.5 min-[500px]:mx-4" aria-labelledby="visual-result-next-actions">
          ...
          <ul className="mt-1.5 grid gap-1">
            {model.actionItems.map(item => <li key={item} className="text-[11px] font-bold leading-5 text-ink/68">· {item}</li>)}
          </ul>
        </section>
      )}
      ...
      {model.avoidActions && model.avoidActions.length > 0 && (
        <section className="mx-3.5 mb-3 rounded-[16px] border border-amber-100 bg-amber-50/80 px-3 py-2.5 min-[500px]:mx-4" aria-labelledby="visual-result-avoid-actions">
          ...
          <ul className="mt-1.5 grid gap-1">
            {model.avoidActions.map(item => <li key={item} className="text-[11px] font-bold leading-5 text-amber-950/72">· {item}</li>)}
          </ul>
        </section>
      )}
