...
export interface ThreeAquariumProps {
  aquarium: Aquarium;
...
function CameraFraming({ length, width, height, framing, targetPosition }: { length: number; width: number; height: number; framing: 'contain' | 'stage-cover'; targetPosition: THREE.Vector3 | null }) {
  ...
  useEffect(() => {
    ...
    const position = getAquariumCameraFrame({ length, width, height, aspect: Math.max(0.35, size.width / Math.max(1, size.height)), framing });
