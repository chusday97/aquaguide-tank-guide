  ...
  return (
    ...
    >
      ...
      <div className="mb-2 flex items-start justify-between gap-2 px-1">
        ...
        <TagPill tone={difficulty === '新手' ? 'normal' : 'warning'}>{Boolean(i18n.language?.startsWith('en')) ? (difficulty === '新手' ? 'Beginner' : 'Advanced') : difficulty}</TagPill>
      </div>
      ...
      <div className="mt-2 grid gap-1.5 rounded-[12px] bg-bg/70 px-2 py-2 text-[10px] font-bold leading-tight text-ink/55">
        <span>{Boolean(i18n.language?.startsWith('en')) ? 'Tank: ' : '缸型：'}{bestFor}</span>
        <span>{Boolean(i18n.language?.startsWith('en')) ? 'Size: ' : '尺寸：'}{tankSize}</span>
        <span>{Boolean(i18n.language?.startsWith('en')) ? 'Environment: ' : '环境：'}{environmentSummary}</span>
        <span>{Boolean(i18n.language?.startsWith('en')) ? 'Hardscape: ' : '造景：'}{layoutSummary}</span>
        <span>{Boolean(i18n.language?.startsWith('en')) ? 'Livestock: ' : '生物：'}{livestockSummary}</span>
      </div>
    </button>
  );
}
