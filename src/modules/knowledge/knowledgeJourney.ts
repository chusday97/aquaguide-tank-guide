import type { KnowledgeJourney, KnowledgeObjectId, KnowledgeUrgency } from '../../types';
...
export function buildKnowledgeJourney(objectId: KnowledgeObjectId, observation: KnowledgeObservation): KnowledgeJourney {
  ...
  return {
    ...
    contextFacts: [],
    emergencyActions: isUrgent ? [{
      id: 'open-priority-guide',
      title: '打开优先处理指引',
      instruction: '先查看现有的低风险检查步骤，再决定是否调整设备或水体。',
      reviewStatus: 'pending',
      sourceIds: observation.topicId ? [observation.topicId] : [],
    }] : [],
    clarifyingQuestions: [{
      id: 'visible-observation',
      prompt: observation.label,
      options: [{ id: observation.id, label: observation.label }],
    }],
    possibleCauses: [],
    avoidActions: [],
