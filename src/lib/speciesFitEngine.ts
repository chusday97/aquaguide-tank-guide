...
const getCompatibilityRisk = (species: Fish, currentLivestock: Array<{ species?: Fish; record?: { quantity?: number } }>): SpeciesFitItem | null => {
  const validLivestock = currentLivestock.filter((item): item is { species: Fish; record?: { quantity?: number } } => (
    Boolean(item?.species?.id) && item.species?.id !== species.id && isAnimalSpecies(item.species)
  ));
...
export const evaluateSpeciesForAquarium = (
  ...
  const lifeType = getLifeType(species);
  const targetIsPlant = lifeType === 'plant' || isAquaticPlantSpecies(species);
  ...
  const missingCoreSpeciesData = targetIsAnimal
    ? !species.category || !species.waterTemperature || !species.tankSize
    : targetIsPlant
      ? !species.category || !species.waterTemperature
      : targetIsHardscape
        ? !species.category
        : true;

  if (targetIsHardscape) {
    matchedItems.push({
      type: 'hardscape_non_livestock',
      title: '硬景不计入活体密度',
      detail: '硬景不会参与鱼类缸容、活体密度或捕食关系判断。',
    });
    confirmations.push({
      type: 'hardscape_placement_check',
      title: '需要确认硬景材质与摆放',
      detail: '加入前请确认材质对水质的影响、尖锐边缘以及摆放稳定性。',
      severity: 'low',
    });
    score += 8;
  } else {
  ...
  if (targetIsUnknown) {
    confirmations.push({
      type: 'unknown_life_type',
      title: '需要确认对象类型',
      detail: '当前资料无法可靠判断这是鱼类、无脊椎、水草还是硬景，因此不执行活体密度或混养规则。',
    });
  }
  ...
  } else if (targetIsPlant) {
    matchedItems.push({
      type: 'plant_non_livestock',
      title: '水草不计入活体密度',
      detail: '水草不会使用鱼类最低缸容、活体密度或捕食关系规则。',
    });
    score += 6;
  }

  if (targetIsAnimal || targetIsPlant) {
    ...
    if (phSensitive && species.phLevel && phRange) {
      ...
      confirmations.push({
        type: 'missing_ph',
        title: '敏感对象建议实测水质',
        detail: `环境线索仅显示“${waterProfile.tendency === 'acidic' ? '可能偏酸' : waterProfile.tendency === 'alkaline' ? '可能偏碱' : waterProfile.tendency === 'marine' ? '海水环境' : '倾向不明确'}”，不能代替 pH 实测；参考范围为 ${species.phLevel}。`,
        severity: 'low',
      });
      score -= 2;
    }
  ...
  if (targetIsPlant && /CO2|强光|水草泥/i.test(textOf(species)) && !hasEquipment(aquarium, '水草灯')) {
  ...
  if (targetIsAnimal) {
    ...
    if (compatibilityRisk?.severity === 'high') {
      ...
      matchedItems.push({ type: 'empty_tank', title: '暂无混养冲突', detail: '当前鱼缸没有其他活体生物，不触发混养冲突。' });
      score += 8;
    } else {
  ...
  if (targetIsAnimal || targetIsPlant) {
    ...
    if (species.difficulty === 'Hard') {
      warnings.push({ type: 'hard_species', title: '养护难度较高', detail: '该对象对经验和稳定性要求更高。', severity: 'low' });
      score -= 10;
    }
  }
  if (targetIsAnimal && species.temperament === 'Peaceful') score += 5;
  if (targetIsAnimal && species.housingMode === '建议单养' && otherLivestock.length > 0) {
    warnings.push({ type: 'single_housing', title: '更适合单养', detail: '该物种更适合单独规划缸位。', severity: 'medium' });
    score -= 15;
  }

  const unresolvedAquariumWater = !targetIsHardscape && aquariumWaterType === 'unknown';
  const unresolvedSpeciesWater = !targetIsHardscape && speciesWaterType === 'unknown';
  const status: SpeciesFitStatus = hardBlocks.length > 0
    ? 'unsuitable'
    : matchedItems.length === 0 || unresolvedAquariumWater || unresolvedSpeciesWater || missingCoreSpeciesData
      ? 'unknown'
      : warnings.length > 0 || confirmations.length > 0
        ? 'adjustable'
        : 'suitable';

  const suitableSummary = targetIsPlant
    ? '已通过水体、温度和水草养护基础条件筛选；未使用鱼类缸容、密度或捕食规则。'
    : targetIsHardscape
      ? '硬景不参与活体密度或捕食判断；已通过当前可确认的非活体规则。'
