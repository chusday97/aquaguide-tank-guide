...
export const hydrateReviewedCompatibilityEvidence = async (fresh = false) => {
  try {
    ...
    const gitSnapshot = await loadGitRuntimeAuthoritySnapshot(fresh);
    if (gitSnapshot?.generatedAt && gitSnapshot.compatibility) return applyReviewedCompatibilityBootstrap(gitSnapshot.compatibility);
