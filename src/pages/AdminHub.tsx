...
export default function AdminHub() {
  ...
  const publishRuntimeSnapshot = async () => {
    if (!window.confirm('生成 Git 发布快照？只会导出当前 Published Product/Care 与 reviewed Compatibility；Draft、审核笔记和本地文件路径不会进入公开快照。')) return;
    setSafetyBusy('publish'); setSafetyMessage('');
    try {
      const result = await publishLocalRuntimeAuthoritySnapshot();
      setSafetyMessage(`Git 发布快照已生成 · Product ${result.counts.species} · Care ${result.counts.care} · Profile ${result.counts.profiles} · Pair ${result.counts.pairRules}`);
    } catch (cause) {
      setSafetyMessage(cause instanceof Error ? cause.message : 'Git 发布快照没有生成。');
    } finally { setSafetyBusy(''); }
  };
