...
import { ArchiveRestore, ArrowLeft, Clock3, Database, FileOutput, HardDriveDownload, Loader2, RefreshCw, Search, ShieldCheck } from 'lucide-react';
import { useLocation, useNavigate } from 'react-router-dom';
import { isSupabaseConfigured } from '../lib/supabaseClient';
import { createLocalAdminBackup, getLocalAdminPersistenceStatus, getLocalAdminSafetySnapshot, isLocalAdminFileMode, publishLocalRuntimeAuthoritySnapshot, restoreLocalAdminBackup, type LocalAdminSafetySnapshot } from '../services/admin/local-file-persistence';
import {
...
export default function AdminHub() {
  ...
  const [safetyBusy, setSafetyBusy] = useState<'backup' | 'restore' | 'publish' | ''>('');
  ...
  const publishRuntimeSnapshot = async () => {
    if (!window.confirm('生成待提交的 Git 运行时快照？这一步只写入仓库工作树，不会自动 commit、push 或部署；只会导出当前 Published Product/Care 与 reviewed Compatibility，Draft、审核笔记和本地文件路径不会进入运行时快照。')) return;
    setSafetyBusy('publish'); setSafetyMessage('');
    try {
      const result = await publishLocalRuntimeAuthoritySnapshot();
      setSafetyMessage(`运行时快照已生成 · 尚未提交 Git / 部署 · Product ${result.counts.species} · Care ${result.counts.care} · Profile ${result.counts.profiles} · Pair ${result.counts.pairRules}`);
    } catch (cause) {
      setSafetyMessage(cause instanceof Error ? cause.message : '运行时快照没有生成。');
    } finally { setSafetyBusy(''); }
  };
