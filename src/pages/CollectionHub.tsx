...
import type { CollectionModule } from '../modules/collection/collection.types';
import { getCollectionSnapshot, subscribeToCollection } from '../services/collection/collection.service';

type LiveCollectionModule = Exclude<CollectionModule, 'achievements'>;

const moduleRoutes: Record<LiveCollectionModule, string> = {
      ...
      <div className="mt-1 flex items-center justify-center gap-2" aria-label={isEn ? 'Choose collection module' : '选择水族册模块'}>
        {items.map((_item, index) => (
          <button
            ...
            className={`h-2 rounded-full transition-all focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-emerald-600 motion-reduce:transition-none ${index === activeIndex ? 'w-7 bg-emerald-700' : 'w-2 bg-slate-300 hover:bg-slate-400'}`}
          />
        ))}
      </div>
      <p className="type-meta mt-2 text-center text-ink/42">
...
function CollectionModuleCard({
  ...
  const openModule = () => navigate(moduleRoutes[id]);

  return (
    <section
      data-collection-module={id}
      className="flex h-full min-h-0 min-w-0 flex-col rounded-[var(--ui-radius-surface)] border border-white/90 bg-white p-4 text-left shadow-[var(--ui-shadow-card)]"
    >
      <button
        type="button"
        onClick={openModule}
      ...
      >
        ...
        <span className="type-card-title min-w-0 flex-1 text-ink">
          {title}
          <span className="type-meta ml-2 text-ink/45">· {count}</span>
        </span>
        <ChevronRight className="h-5 w-5 shrink-0 text-ink/25 transition-transform group-hover:translate-x-0.5 motion-reduce:transition-none" />
      </button>
      <span className="mt-3 flex min-h-0 w-full flex-1 flex-col overflow-hidden rounded-[var(--ui-radius-card)] border border-slate-100 bg-[#fbfcfb]">
        {children}
      </span>
      {remainingCount > 0 && (
        <button
...
function PreviewEmpty({
  ...
  return (
    <span className="flex min-h-[210px] flex-1 flex-col items-center justify-center px-5 text-center">
      <span className="flex h-12 w-12 items-center justify-center rounded-[var(--ui-radius-control)] bg-white text-ink/25 shadow-sm">{icon}</span>
      <span className="type-card-title mt-3 text-ink">{title}</span>
      <span className="type-meta mt-1 max-w-[280px] leading-5 text-ink/42">{description}</span>
    </span>
  );
}
...
export default function CollectionHub() {
  ...
  const openItem = (module: LiveCollectionModule, itemId: string) => {
    if (module === 'memorial') {
  ...
  return (
    <div className="collection-hub page-frame mx-auto flex w-full min-w-0 max-w-[1180px] flex-col gap-4 pb-24">
      <header className="px-1 py-1">
        <h1 className="type-page-title text-ink">{isEn ? 'My Collection' : '我的水族册'}</h1>
        <p className="type-meta mt-1 text-ink/48">{isEn ? 'Wishlist · Care · Memorials' : '种草 · 养护 · 纪念'}</p>
      </header>
      ...
      <CollectionCarousel isEn={isEn}>
        ...
        >
          {wishlistFishes.length ? wishlistFishes.map(fish => (
            ...
            >
              <span className="flex h-[62px] w-[104px] shrink-0 items-center justify-center overflow-hidden rounded-[var(--ui-radius-control)] bg-white">
                <ResilientImage
              ...
              <span className="min-w-0">
                <span className="type-card-title block truncate text-ink">{fish.name}</span>
                <span className="type-meta mt-1 inline-flex rounded-full bg-rose-50 px-2 py-0.5 text-rose-600">{fish.category}</span>
              </span>
            </button>
          )) : (
            <PreviewEmpty
        ...
        >
          {careTopics.length ? careTopics.map(topic => (
            ...
            >
              <span className="h-[82px] overflow-hidden rounded-[var(--ui-radius-control)] bg-white">
                <ResilientImage
              ...
              <span className="min-w-0">
                <span className="type-card-title line-clamp-2 text-ink">{topic.title}</span>
                <span className="type-meta mt-1 line-clamp-2 leading-4 text-ink/45">{topic.summary}</span>
              </span>
            </button>
          )) : (
            <PreviewEmpty
        ...
        >
          {recentMemorials.length ? recentMemorials.map(record => {
            ...
            return (
              ...
              >
                <span className="flex h-[82px] items-center justify-center overflow-hidden rounded-[var(--ui-radius-control)] bg-slate-100 grayscale">
                  {fish ? (
                    <ResilientImage
                      src={getSpeciesVisualSources(fish).thumbnail}
                      alt=""
                ...
                <span className="min-w-0">
                  <span className="type-card-title block truncate text-ink">{fish?.name || (isEn ? 'Species unavailable' : '物种信息不可用')}</span>
                  <span className="type-meta mt-1 block text-ink/45">{formatDate(record.date)}</span>
                  <span className={`type-meta mt-2 inline-flex rounded-full px-2 py-0.5 ${record.reason ? 'bg-emerald-50 text-emerald-700' : 'bg-amber-50 text-amber-700'}`}>
      ...
      <aside
        ...
        data-feature-status="building"
        className="flex items-start gap-3 rounded-[var(--ui-radius-card)] border border-slate-200/80 bg-slate-50 px-4 py-3 text-slate-500"
        aria-label={isEn ? 'Coming next: Achievements' : '即将推出：成就勋章'}
      >
        <span className="flex h-10 w-10 shrink-0 items-center justify-center rounded-[var(--ui-radius-control)] bg-white text-slate-400 shadow-sm">
          <Medal className="h-5 w-5" />
        </span>
        <span className="min-w-0">
          <span className="type-card-title block text-slate-600">{isEn ? 'Coming next · Achievements' : '即将推出 · 成就勋章'}</span>
          <span className="type-meta mt-1 block text-slate-500">{isEn ? 'Care milestones will live here when the feature is ready.' : '功能开放后，这里会记录长期养护里程碑；现在不会占用主轮播。'}</span>
        </span>
      </aside>
    </div>
  );
