...
export function SpeciesLanding() {
  ...
  return <SeoPageShell>
    ...
    {hasBehavior && <section id="behavior" className="seo-section" aria-labelledby="behavior-title"><SectionHeading id="behavior-title" number={sectionNumber('behavior')} eyebrow="" title={labels.behavior} /><div className="seo-life-grid seo-stagger mt-8">{lifeAnswers.map(item => <LifeAnswerCard key={item.question} {...item} />)}</div></section>}
    {profile.editorial?.habitat && <section id="habitat" className="seo-section" aria-labelledby="habitat-title"><SectionHeading id="habitat-title" number={sectionNumber('habitat')} eyebrow="" title={labels.habitat} /><div className="seo-card mx-auto mt-8 max-w-[680px] p-6"><p className="seo-body font-bold">{profile.editorial.habitat.summary}</p>{profile.editorial.habitat.details && <ul className="mt-5 grid gap-3 text-sm font-normal leading-7 text-ink/62">{profile.editorial.habitat.details.map(detail => <li key={detail} className="flex gap-3"><span className="mt-2 h-1.5 w-1.5 shrink-0 rounded-full bg-accent/55" />{detail}</li>)}</ul>}</div></section>}
    {(profile.editorial?.feeding || profile.editorial?.maintenance) && <section id="care" className="seo-section" aria-labelledby="care-title"><SectionHeading id="care-title" number={sectionNumber('care')} eyebrow="" title={labels.care} /><div className="mt-8 grid gap-3">{[profile.editorial.feeding, profile.editorial.maintenance].filter((section): section is PublishedContentSection => Boolean(section)).map(section => <EditorialSection key={section.id} section={section} />)}</div></section>}
