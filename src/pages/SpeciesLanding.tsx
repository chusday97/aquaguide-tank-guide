...
export function SpeciesLanding() {
  ...
  return <SeoPageShell>
    ...
    {groupVariants.length > 1 && <section id="variants" className="seo-section" aria-labelledby="variants-title"><SectionHeading id="variants-title" number={sectionNumber('variants')} eyebrow={labels.variants} title={labels.variants} /><div className="seo-stagger mt-8 grid min-w-0 grid-cols-2 gap-3 md:grid-cols-4">{groupVariants.map(variant => { const active = variant.id === fish.id; const pilotVariant = getSpeciesLandingPilotRecord({ id: variant.id, name: variant.name, scientificName: variant.scientificName }); const variantAsset = variant.image || (assetPreviewEnabled ? toPreviewAsset(variant, pilotVariant.asset.variantCard) : undefined); return <Link key={variant.id} to={variantPath(variant.id)} aria-current={active ? 'page' : undefined} className={`seo-focus min-w-0 rounded-[20px] border p-3 transition-colors ${active ? 'border-emerald-300 bg-emerald-50' : 'border-border bg-white hover:border-emerald-200'}`}><MediaFrame asset={variantAsset} label={labels.imageUnavailable} alt={variantAsset ? variantAsset.altZh : `${variant.name}（${variant.scientificName}）`} fallbackTitle={variant.name} className="min-h-[120px] rounded-[16px] p-0 shadow-none" /><p className="mt-3 truncate text-sm font-bold text-ink">{variant.name}</p><p className="seo-meta mt-1 truncate">{variant.scientificName}</p>{variant.difference && <p className="mt-2 text-xs font-normal leading-5 text-ink/60">{variant.difference}</p>}{active && <span className="mt-2 inline-flex rounded-full bg-white px-2 py-1 text-[10px] font-bold text-accent">{labels.current}</span>}</Link>; })}</div></section>}
