...
const labels = {
  back: '首页', freshwater: '淡水', saltwater: '海水', difficulty: '养护难度', easy: '极易', medium: '中等', hard: '困难', favorite: '收藏', saved: '已收藏', chapters: '章节导航', overview: '一眼了解', behavior: '它如何生活', habitat: '适合怎样的环境', care: '日常怎么养', variants: '外观与品系', faq: '常见问题', showDetails: '展开详情', hideDetails: '收起详情', tool: '它适合我的鱼缸吗？', toolDetail: '比较它与我的鱼缸是否匹配。', openTool: '检查我的鱼缸', temp: '水温', ph: 'pH', minTank: '最低缸体', size: '体型', temperament: '性情', imageUnavailable: '图片暂时不可用', imageFailed: '图片暂时不可用', related: '相关内容', current: '当前', sources: '资料来源', sourceText: '', returnSearch: '回到首页', missing: '没有找到这个物种页面。', learnMore: '回到首页继续浏览。',
} as const;
...
const toPreviewAsset = (fish: Pick<Fish, 'id'>, source: SpeciesLandingAssetUse): PublishedSpeciesAsset | undefined => {
...
function MediaFrame({ asset, label, alt, fallbackTitle, failed, onFallback, className = '', mode = 'hero' }: { asset?: PublishedSpeciesAsset; label: string; alt: string; fallbackTitle?: string; failed?: boolean; onFallback?: () => void; className?: string; mode?: 'hero' | 'variant-card' }) {
  const isVariantCard = mode === 'variant-card';
  if (!asset || failed) return <SeoAssetFallback label={label} title={fallbackTitle} className={`${isVariantCard ? 'seo-variant-card__fallback' : 'seo-media-fallback rounded-[28px] border border-dashed border-emerald-200 bg-[radial-gradient(circle_at_50%_35%,rgba(175,220,202,.34),transparent_48%),#F3F8F3] p-8 text-center'} ${className}`} />;
  return <div className={`${isVariantCard ? 'seo-variant-card__media' : 'flex min-h-[260px] items-center justify-center overflow-hidden rounded-[28px] border border-white/80 bg-white p-6 shadow-[0_18px_60px_rgba(27,77,62,0.08)]'} ${className}`}><ResilientImage src={asset.src} alt={alt} loading="eager" onFallback={onFallback} className={isVariantCard ? 'seo-variant-card__image' : 'h-full w-full object-contain p-[8%]'} /></div>;
}
...
const personalizeFaq = (text: string, baseName: string, speciesName: string) => (
  speciesName === baseName ? text : text.replaceAll(baseName, speciesName)
);
...
export function SpeciesLanding() {
  ...
  // Public breadcrumbs use the published category name. Internal care taxonomy
  // labels such as "除藻生物" do not describe the public category route.
  const breadcrumbCategory = baseSpecies.category;
  ...
  const heroSignature = groupVariants.find(variant => variant.id === fish.id)?.difference || profile.editorial?.signature;
  ...
  return <SeoPageShell>
    ...
    <SeoHero><div className="seo-hero__media order-2 min-w-0 lg:order-1"><MediaFrame asset={heroAsset} label={heroImageFailed ? labels.imageFailed : labels.imageUnavailable} alt={imageAlt} fallbackTitle={fish.name} failed={heroImageFailed} onFallback={() => setHeroImageFailed(true)} className="min-h-[280px] md:min-h-[470px]" /></div><div className="seo-hero__content order-1 min-w-0 lg:order-2"><p className="seo-eyebrow">{categoryText} · {waterTypeText}</p><h1 className="seo-hero__title break-words">{fish.name}</h1><p className="seo-hero__scientific break-words">{fish.scientificName}</p>{heroSignature && <p className="seo-lead mt-6">{heroSignature}</p>}<div className="mt-6 flex flex-wrap gap-2"><span className="rounded-full border border-emerald-100 bg-emerald-50 px-3 py-1.5 text-[13px] font-bold text-emerald-900">{localizeDifficulty(fish)}</span><span className="rounded-full border border-sky-100 bg-sky-50 px-3 py-1.5 text-[13px] font-bold text-sky-900">{waterTypeText}</span><span className="rounded-full border border-border bg-white px-3 py-1.5 text-[13px] font-bold text-ink/62">{localizeTemperament(fish)}</span></div>{actionFeedback && <p className="seo-meta mt-4 font-bold text-accent" role="status" aria-live="polite">{actionFeedback}</p>}<div className="mt-7 flex flex-wrap gap-3"><button type="button" onClick={() => void toggleFavorite()} disabled={favoriteSaving} aria-pressed={isFavorite} className="seo-action seo-focus border border-border bg-white text-ink/70 hover:border-rose-200 hover:text-rose-700 disabled:cursor-wait disabled:opacity-60">{favoriteSaving ? <Loader2 className="h-4 w-4 animate-spin" aria-hidden="true" /> : <Heart className={isFavorite ? 'h-4 w-4 fill-current text-rose-600' : 'h-4 w-4'} aria-hidden="true" />}{isFavorite ? labels.saved : labels.favorite}</button></div></div></SeoHero>

    <section id="overview" className="seo-section" aria-labelledby="stats-title"><SectionHeading id="stats-title" number={sectionNumber('overview')} eyebrow="核心参数" title={labels.overview} /><SeoDataRail items={stats.map(item => ({ ...item, icon: <StatIcon label={item.label} /> }))} />{profile.editorial?.overview && <div className="seo-card mx-auto mt-8 max-w-[680px] p-5 md:p-6"><p className="seo-body">{profile.editorial.overview.summary}</p></div>}</section>

    {hasBehavior && <section id="behavior" className="seo-section" aria-labelledby="behavior-title"><SectionHeading id="behavior-title" number={sectionNumber('behavior')} eyebrow="生活方式" title={labels.behavior} /><div className="seo-life-grid seo-stagger mt-8">{lifeAnswers.map(item => <LifeAnswerCard key={item.question} {...item} />)}</div></section>}
    {profile.editorial?.habitat && <section id="habitat" className="seo-section" aria-labelledby="habitat-title"><SectionHeading id="habitat-title" number={sectionNumber('habitat')} eyebrow="环境" title={labels.habitat} /><div className="seo-card mx-auto mt-8 max-w-[680px] p-6"><p className="seo-body font-bold">{profile.editorial.habitat.summary}</p>{profile.editorial.habitat.details && <ul className="mt-5 grid gap-3 text-sm font-normal leading-7 text-ink/62">{profile.editorial.habitat.details.map(detail => <li key={detail} className="flex gap-3"><span className="mt-2 h-1.5 w-1.5 shrink-0 rounded-full bg-accent/55" />{detail}</li>)}</ul>}</div></section>}
    {(profile.editorial?.feeding || profile.editorial?.maintenance) && <section id="care" className="seo-section" aria-labelledby="care-title"><SectionHeading id="care-title" number={sectionNumber('care')} eyebrow="日常" title={profile.editorial.feeding && !profile.editorial.maintenance ? '取食方式' : labels.care} /><div className="mt-8 grid gap-3">{[profile.editorial.feeding, profile.editorial.maintenance].filter((section): section is PublishedContentSection => Boolean(section)).map(section => <EditorialSection key={section.id} section={section} />)}</div></section>}
    {groupVariants.length > 1 && <section id="variants" className="seo-section" aria-labelledby="variants-title"><SectionHeading id="variants-title" number={sectionNumber('variants')} eyebrow={labels.variants} title={labels.variants} /><div className="seo-variant-grid seo-stagger mt-8">{groupVariants.map(variant => { const active = variant.id === fish.id; const pilotVariant = getSpeciesLandingPilotRecord({ id: variant.id, name: variant.name, scientificName: variant.scientificName }); const variantAsset = variant.image || (assetPreviewEnabled ? toPreviewAsset(variant, pilotVariant.asset.variantCard) : undefined); return <Link key={variant.id} to={variantPath(variant.id)} aria-current={active ? 'page' : undefined} className={`seo-variant-card seo-focus ${active ? 'seo-variant-card--active' : ''}`}><MediaFrame mode="variant-card" asset={variantAsset} label={labels.imageUnavailable} alt={variantAsset ? variantAsset.altZh : `${variant.name}（${variant.scientificName}）`} fallbackTitle={variant.name} /><div className="seo-variant-card__copy"><p className="seo-variant-card__name">{variant.name}</p><p className="seo-meta seo-variant-card__scientific">{variant.scientificName}</p>{variant.difference && <p className="seo-variant-card__difference">{variant.difference}</p>}{active && <span className="seo-variant-card__current">{labels.current}</span>}</div></Link>; })}</div></section>}
    ...
    {profile.faq.length > 0 && <section id="faq" className="seo-section" aria-labelledby="faq-title"><SectionHeading id="faq-title" number={sectionNumber('faq')} eyebrow={labels.faq} title={labels.faq} /><div className="mt-8 grid gap-3">{profile.faq.map(item => <SeoDisclosure key={item.id} label={personalizeFaq(item.question, baseSpecies.name, fish.name)}><p className="seo-body">{personalizeFaq(item.answer, baseSpecies.name, fish.name)}</p></SeoDisclosure>)}</div></section>}
    {profile.relatedLinks.length > 1 && <SeoRelatedLinks title={labels.related} links={profile.relatedLinks} />}
