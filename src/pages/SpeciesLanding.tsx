...
export function SpeciesLanding() {
  ...
  return <SeoPageShell>
    ...
    {hasBehavior && <section id="behavior" className="seo-section" aria-labelledby="behavior-title"><SectionHeading id="behavior-title" number={sectionNumber('behavior')} eyebrow={labels.behavior} title={labels.behavior} /><div className="seo-life-grid seo-stagger mt-8">{lifeAnswers.map(item => <LifeAnswerCard key={item.question} {...item} />)}</div></section>}
    ...
    {(profile.editorial?.feeding || profile.editorial?.maintenance) && <section id="care" className="seo-section" aria-labelledby="care-title"><SectionHeading id="care-title" number={sectionNumber('care')} eyebrow={profile.editorial.feeding && !profile.editorial.maintenance ? '取食' : labels.care} title={profile.editorial.feeding && !profile.editorial.maintenance ? '已确认的取食方式' : labels.care} /><div className="mt-8 grid gap-3">{[profile.editorial.feeding, profile.editorial.maintenance].filter((section): section is PublishedContentSection => Boolean(section)).map(section => <EditorialSection key={section.id} section={section} />)}</div></section>}
