...
export function SpeciesLanding() {
  ...
  return <SeoPageShell>
    ...
    <section className="seo-section" aria-labelledby="stats-title"><SectionHeading id="stats-title" number={sectionNumber('overview')} eyebrow={labels.overview} title={labels.overview} /><SeoDataRail items={stats.map(item => ({ ...item, icon: <StatIcon label={item.label} /> }))} />{profile.editorial?.overview && <p className="seo-lead mx-auto mt-8 max-w-[680px]">{profile.editorial.overview.summary}</p>}</section>
