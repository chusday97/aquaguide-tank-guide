...
export function SpeciesLanding() {
  ...
  return <SeoPageShell>
    ...
    {hasBehavior && <section id="behavior" className="seo-section" aria-labelledby="behavior-title"><SectionHeading id="behavior-title" number={sectionNumber('behavior')} eyebrow={labels.behavior} title={labels.behavior} /><div className="mt-8 grid gap-4 md:grid-cols-3">{lifeAnswers.map(item => <LifeAnswerCard key={item.question} {...item} />)}</div></section>}
    {profile.editorial?.habitat && <section id="habitat" className="seo-section" aria-labelledby="habitat-title"><SectionHeading id="habitat-title" number={sectionNumber('habitat')} eyebrow={labels.habitat} title={profile.editorial.habitat.heading} description={labels.chapterSignatures.habitat} /><div className="mt-8 grid gap-6 lg:grid-cols-[.75fr_1.25fr] lg:items-stretch"><div className="flex min-h-[240px] items-end rounded-[28px] bg-[radial-gradient(circle_at_72%_20%,rgba(181,224,207,.62),transparent_38%),linear-gradient(145deg,#E8F1EA,#F7F8F2)] p-6"><p className="max-w-[260px] font-serif text-2xl font-bold leading-tight text-accent/85">{labels.chapterSignatures.habitat}</p></div><div className="seo-card seo-large-card p-6"><p className="seo-body font-bold">{profile.editorial.habitat.summary}</p>{profile.editorial.habitat.details && <ul className="mt-5 grid gap-3 text-sm font-normal leading-7 text-ink/62">{profile.editorial.habitat.details.map(detail => <li key={detail} className="flex gap-3"><span className="mt-2 h-1.5 w-1.5 shrink-0 rounded-full bg-accent/55" />{detail}</li>)}</ul>}</div></div></section>}
