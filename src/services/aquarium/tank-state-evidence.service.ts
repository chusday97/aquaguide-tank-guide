...
export const buildTankObservationsFromDiagnosisRecords = (
  ...
  records.filter(record => record.aquariumId === aquariumId).forEach(record => {
    ...
    const { userDescription: _freeTextDescription, ...structuredAnswers } = answers;
    const answerText = Object.values(structuredAnswers).join(' ');
    const addWaterSignals = () => {
      const waterText = `${answers.waterLook || ''} ${answers.cloudyWater || ''} ${answers.mainProblem || ''}`;
      if (includesOne(waterText, ['明显浑浊', '轻微浑浊', '发白', '发绿', '水变浑'])) add(record, 'cloudy_water');
      if (includesOne(waterText, ['有异味', '明显异味'])) add(record, 'odor');
    };

    addWaterSignals();
    if (includesOne(answerText, ['浮头', '呼吸急促', '急促呼吸', '喘气'])) add(record, 'respiratory_distress');
    if (includesOne(answerText, ['明显追咬', '持续霸占区域', '被追赶', '追咬打架'])) add(record, 'persistent_chasing');
    if (includesOne(answerText, ['长时间躲藏', '趴底不动', '趴缸', '躲藏'])) add(record, 'hiding_pressure');
    if (includesOne(answerText, ['连续一天不吃', '连续多天不吃', '全缸都不吃', '不吃食', '鱼不吃'])) add(record, 'appetite_drop');
    if (includesOne(answerText, ['咬伤鳍条', '烂尾烂鳍'])) add(record, 'injury');
    if (includesOne(answerText, ['死亡多条', '连续死了多条', '2-3 条', '多只', '连续死亡'])) add(record, 'multiple_deaths');
