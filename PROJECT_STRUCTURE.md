...
- `src/App.tsx`：设备级应用壳、导航与路由；公开 SEO 路由与应用 Shell 分离。
...
- `src/data/publishedSpeciesProfile.ts`：只聚合已允许公开的物种百科内容、审核习性、素材、Variant 摘要和相关链接；不读取鱼缸或计算适配结果。
- `src/data/publishedPublicSeo.ts`：Category/Care Guide 本地 fail-closed 聚合；未审核内容不进入公开页。
- `src/pages/MarketingLanding.tsx`、`src/pages/CategoryLanding.tsx`、`src/pages/CareGuideLanding.tsx`：公开 SEO 根页、分类页和养护指南准备态。
- `src/services/seo/seo-document.service.ts`：公开路由的 title、description、robots、canonical 和 JSON-LD 管理。
...
- `src/services/species/species-landing.service.ts`：公开物种页的本地目录选择、Base/Variant 分组和重复记录隔离。
- `docs/CONTEXT_ROUTING.md`：项目文档 canonical home 与 Species SEO 当前交接入口。
...
- `scripts/test-species-variant-card-contract.mjs`：验证 Species 品系卡片 4:3 媒体比例、contain 图片、说明结构、响应式列数和孔雀鱼基础案例。
...
- `docs/species-seo-asset-source-manifest.json` / `docs/species-seo-asset-review-sheet.md`：项目内 SEO 素材来源、hash、用途和人工确认门禁。
...
- `docs/public-seo-visual-audit-20260913.md`：Species 三档对照图的视觉审计与素材人工确认状态。
...
- `src/data/seoEvidenceBindings.ts`：本地 SEO 来源绑定清单、确定性指纹、状态门禁与报告接口。
- `scripts/test-seo-evidence-bindings.ts`：来源变更、review 状态降级、素材 SHA-256 和 Published Profile fail-closed 回归测试。
- `docs/seo-evidence-binding-report.md`：当前试点逐条“页面表达 → 来源 ID → 状态 → 确认时间”报告。
