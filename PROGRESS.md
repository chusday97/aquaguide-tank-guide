...
- [x] 修正响应式契约测试：由字符串匹配 `44px` 改为数值断言控件最小高度 `>=44px`，保留严格浏览器尺寸门禁；本地回归重新执行中。证据将随本次提交更新。

- [x] Run `34772008026` 已定位根因：Species “收藏”按钮计算高度为 `43.999969px`，严格浮点检查因此失败；将公开操作控件最小高度调整为 45px，避免浏览器布局精度低于 44px。证据为 `EVD-20260914-159`。
- [ ] 45px 修复尚未在 GitHub Actions 新 SHA 上验证；需等待下一次浏览器门禁结果。

- [x] Run `34771775858` 证实新断言发现孔雀鱼页面存在 1 个小于 44px 的交互目标；已增强失败信息，下一次 CI 会输出具体元素标签、文字、链接和实际尺寸。证据为 `EVD-20260914-158`。
- [ ] 具体元素尚未由 CI 日志返回前，不猜测修复对象；页面样式暂不做盲改。
...
- [x] GitHub Actions run `34766143025` 在 SHA `ad4a690a9c694ac0ce400caf459397704da6cbac` 通过新增滚动、返回、章节锚点和 reduced-motion 检查；artifact 仍包含 12 张原始截图和 4 张对照图，证据为 `EVD-20260913-141`。
- [ ] 交互延迟观察器本次运行返回 `null`，未伪造性能结论；Critic 仍无可读正文，Figma Canonical、性能完整基线和索引门禁保持未完成。
...
- [ ] 普通公开访问仍显示素材回退；Hero 与 variant-card 用途尚未批准，不能把审核预览当作 Published 素材。

## 2026-09-13 600px 视口探测结果

- [x] 尝试通过内置 Chrome 页面脚本调整到 600px 时，`window.resizeTo` 在隔离上下文中不存在并抛出 `TypeError`；页面保持原桌面视口且内容正常，证据为 `EVD-20260913-121`。
- [ ] 该结果不计为 600px 验收通过；仍需真实可控的 600px 浏览器视口或系统 Chrome 环境恢复后补验。
...
- [x] 内置浏览器复核宝莲灯、虾螺蟹分类页和新鱼入缸指南：内容层级、公开 Header、物种入口、准备态和能力入口均可读，证据为 `EVD-20260912-068`。
- [ ] 系统 Chrome 三档、性能/reduced-motion、Figma Canonical 和可读独立 Critic仍未完成。

## 2026-09-12 宝莲灯工具入口回归

- [x] 内置浏览器点击宝莲灯“检查我的鱼缸”后进入兼容工具，`species=sp_0432` 与 `source=species-profile` 保留，并自动带入宝莲灯，证据为 `EVD-20260912-069`。
- [ ] 返回链路和系统 Chrome 多视口仍待补；页面继续 `noindex,follow`。

## 2026-09-12 宝莲灯工具返回回归

- [x] 兼容工具返回按钮回到 `/species/sp_0432`，宝莲灯标题、参数、行为、来源和公开导航恢复正常，证据为 `EVD-20260912-070`。
...
- [x] 同一只只读 Critic 已针对当前 HEAD 完成复验请求，但返回 `items: []`，没有可读六维正文；按门禁记录为审查基础设施阻塞，不计为通过，证据为 `EVD-20260912-061`。
- [x] 工作树检查确认仅保留用户原有的三个未提交文件；本轮未触碰、未暂存、未提交。
- [ ] 可读独立 Critic、系统 Chrome 三档、性能/reduced-motion 和 Figma Canonical 仍未完成；页面继续 `noindex,follow`。
...
- [x] 公开结构、契约、文案、响应式、lint、build 和 diff-check 通过；证据为 `EVD-20260912-048`。
- [ ] 系统 Chrome 三档、性能/reduced-motion、Figma Canonical 和可读独立 Critic 仍未完成。

## 2026-09-12 宝莲灯鱼类工具入口与返回复核

- [x] 内置 Chrome 实际复核宝莲灯页面进入兼容工具并返回；鱼类分类、Product Truth、环境、活动水层、群游和取食内容保持正确，工具携带 `species=sp_0432`，证据为 `EVD-20260912-047`。
- [ ] 可读独立 Critic、系统 Chrome 三档、性能/reduced-motion 和 Figma Canonical 仍未完成。

## 2026-09-12 黄金米虾工具入口与返回复核

- [x] 内置 Chrome 实际复核黄金米虾品系页进入兼容工具并返回；继承生活习性、FAQ、黄色差异、18–28°C、物种 ID 和来源参数均正确，证据为 `EVD-20260912-046`。
- [ ] 可读独立 Critic、系统 Chrome 三档、性能/reduced-motion 和 Figma Canonical 仍未完成。

## 2026-09-12 当前 SHA Critic 复验结果

- [x] 同一只读 Critic 已读取当前版本并完成复验，但返回空 `items`、无可读六维正文；记录为 `EVD-20260912-045`，不计为审查通过。
- [ ] 可读独立 Critic、系统 Chrome 三档、性能/reduced-motion 和 Figma Canonical 仍未完成。
...
- [x] 文案测试、lint 和 diff-check 通过，证据为 `EVD-20260912-042`。
...
## 2026-09-12 分类页单卡布局收口（`8c045c34`）

- [x] 分类页仅有一个已公开基础物种时，改为单列舒适宽度；只有多个物种时才启用双列，消除半宽卡片造成的空白。
- [x] 内置 Chrome 截图确认极火虾卡片已占满约 760px 内容宽度；公开结构、响应式、lint、build 和 diff-check 通过。
- [ ] 本次视觉证据覆盖当前桌面视口，不替代 390/600/1440 自动化、性能或独立 Critic。
...
- [x] 响应式、公开结构、公开文案、TypeScript 与 diff-check 通过。
- [ ] 真实 390px 截图仍待 Mac 解锁后完成；不把 CSS 门禁替代为视觉验收。

## 2026-09-10 浏览器环境复查

- [x] 通过 Codex Computer Use 重新检查本地浏览器环境；系统明确返回 Mac 当前锁定且无法自动解锁。
- [x] 未重复启动浏览器或伪造视觉验收结果；本地代码与静态门禁继续保持可验证状态。
- [ ] 真实 390/600/1440 截图、独立 Critic、Figma Canonical 和发布门禁仍待浏览器解锁及后续条件满足。

## 2026-09-10 公开页面桌面滚动根因修复

- [x] 修复公开 SEO 页面在桌面尺寸下可能继承应用壳 `#root { overflow: hidden; }` 的问题；公开根节点现在明确使用 `height: auto` 与 `overflow: visible`，长页面可以继续由文档滚动。
- [x] 响应式契约新增公开根节点不被应用壳裁切的断言；响应式、结构、文案、公开契约、lint、build 与 diff-check 通过。
- [ ] 尚未获得真实桌面/平板/手机截图；系统浏览器仍受 macOS 进程权限阻塞，Figma Canonical 与可读独立 Critic 仍待完成。
...
- 已验证：稳定提交 `01d71f12` 的 `foundation`、`validate`、GP-001 至 GP-005、Vercel 与 Cloudflare Preview 全部通过；本地/远端/PR/READY Preview 四方 SHA parity 通过。Care 动作、Species 390/600/1440、Public/App 九路由、metadata、noindex 和无溢出均通过；本地 LCP 基线 332ms。
- 托管证据：`https://aquaguide-pwq75dpnl-chusday97s-projects.vercel.app` 的六条公开路由均返回 HTTP 200 与 `x-robots-tag:noindex`。本机浏览器访问该域名仍为 `ERR_CONNECTION_CLOSED`，所以托管截图、网络瀑布和 CWV 尚未完成。
...
- 当前阻塞：独立只读 Critic 在限定等待与收敛提醒后持续处于 `running`，未返回可读六维报告，已关闭并按审查基础设施阻塞处理；托管浏览器证据与 Figma Canonical 也仍未完成。不得解除索引或合并。
- 已完成：新增 fail-closed Preview parity 检查；必须显式提供 `PREVIEW_PR`，并同时匹配本地/远端/PR/READY Preview 的同一 SHA，缺少 PR 或部署时直接失败。
- 下一步：由项目负责人查看 Draft Preview 的桌面/平板/手机视觉；浏览器环境可用后补托管截图/CWV，再完成 Figma Canonical、可读 Critic 和 SEO 页面意图归属。当前 PR 保持 Draft。

## 2026-09-08 UI 分支收敛基础步骤（当前）

- [x] 核对分支关系：`origin/main@64fa58a1` 保持唯一代码事实；历史 `codex/ui-atlas-care-v1@4113050e` 领先70个提交但冻结，不整体合并。
- [x] 从最新 `origin/main` 建立 `codex/ui-action-contract-v1`，只迁移动作语义与独立 `/compatibility` 路由基础。
...
- [x] `f232d884` 修复 Species 相关分类链接：已开放的虾螺蟹分类进入公开分类页，其他分类进入对应图鉴筛选，避免宝莲灯误跳虾螺蟹。
- [x] 内置浏览器复核宝莲灯：页面可滚动；`assetPreview=1` 可见图片与中文 Alt；生活习性、环境章节和能力入口均可读。
- [x] 内置浏览器完成分类 → 极火虾 → 浏览器返回 → 分类页路径；URL、物种页内容和分类页内容均恢复正常。
- [x] 纯 Vite `127.0.0.1:3002` 预览再次完成分类 → Species → 浏览器返回；确认公开页面不需要 API 进程即可渲染和导航。
- [ ] 常规 `npm run dev` 仍受旧 API 端口占用与 tsx IPC 权限影响；后续浏览器巡检固定使用纯 Vite 预览并单独记录该环境缺口。
...
- [x] 在当前可用的内置浏览器预览中补查 Species 长页面：首屏、参数带、生活习性、环境、日常、品系、FAQ、资料来源均可读，章节滚动可达。
...
- [x] 页面组件与公开聚合层文案扫描均通过；未改变证据、Product Truth、路由或索引策略。
- [ ] 三档自动化浏览器、Figma Canonical 和可读独立 Critic 仍未完成。
...
## 2026-09-10 公开页面流程文案去内部化

- [x] 分类页将“进入公开路径”改为“进入已经整理好的物种档案”。
...
- [x] 中文公开文案门禁新增“目前确认的品系差异”和“待审核”流程词扫描，防止审核术语回流到用户页面。
- [x] 公开文案、结构、契约、lint 与 diff-check 回归通过。
- [x] 公开结构回归新增主要 CTA、分类卡、品系卡和鱼缸工具路由断言；该门禁不替代真实点击与返回验收。
...
- [x] 当前 HEAD 的公开SEO文案、结构、素材、证据、契约、响应式、lint、build 和 diff-check 复跑通过，证据为 EVD-20260910-142。
- [ ] 原生 Chrome 通道也返回不可用，结合 Playwright MachPort 失败，最终多视口浏览器门禁暂时阻塞，证据为 EVD-20260910-143。

## 2026-09-10 内置浏览器交互闭环补充

- [x] 实测黄金米虾品系入口可跳转到 `?variant=sp_0030`，URL、H1、黄色图片与中文 Alt 一致。
- [x] 实测浏览器返回可恢复极火虾页面；FAQ 可通过点击展开并显示答案。
- [x] 实测“浏览虾螺蟹分类”可到达分类公开页，Public Header 存在且未进入 App Shell。
- [ ] 以上证据仍不替代系统 Chrome 多视口、reduced-motion、性能指标和可读独立 Critic；页面继续 `noindex,follow`。

## 2026-09-10 宝莲灯鱼类样板复核

- [x] 本地预览服务重启后，宝莲灯页面正常加载；实际可见环境、中层活动、群游与最低群体数量、觅食文案。
- [x] 普通宝莲灯路由未显示待审核图片，图片回退策略和 `noindex,follow` 保持有效。
- [x] 确认一次连接拒绝来自预览服务进程停止，随后新建内置浏览器标签复核成功；不归因于页面路由逻辑。
- [ ] 系统 Chrome 多视口、性能、reduced-motion、独立 Critic 与 Figma Canonical 仍未完成。
- [ ] 同一独立 Critic 已针对当前 HEAD `93a7fad6` 完成只读复验，但 turn 仍无任何可读正文；按门禁记录为审查基础设施阻塞，不计为通过。

## 2026-09-10 独立 Critic 可读报告恢复

- [x] 同一只读 Critic 任务已返回可读六维报告，基准为 `93a7fad6`，并明确区分通过、应修复和阻塞项。
- [x] 已核对 `93a7fad6..HEAD` 只有进度、交接、变更日志和证据档案变更，没有代码变化，因此该报告对当前代码仍具参考效力。
- [ ] Critic 结论不是整体通过：真实多视口运行、部分失败状态验证、完整键盘路径、Figma Canonical 和发布门禁仍未完成。

## 2026-09-12 本地预览与滚动/键盘复核

- [x] 本地 Vite 预览恢复；内置浏览器新标签可加载宝莲灯 `/species/sp_0432`。
...
## 2026-09-12 Critic 当前版本复验结果

- [x] 已沿用既有 Critic 任务复验当前 HEAD `32a7470b`，任务正常结束但 turn 无任何可读正文，证据为 `EVD-20260912-005`。
- [ ] 该空输出不能算独立审查通过；不创建重复 Critic，不解除 `noindex,follow`。待审查输出能力恢复后再复验当前 SHA。
## 2026-09-12 公开导航链路复核
...
## 2026-09-12 当前 HEAD 静态门禁复跑

- [x] `ff2f2ff1` 上 Editorial、Evidence、Public Contract、Responsive Contract、Asset、lint、build 和 diff-check 全部通过，证据为 `EVD-20260912-012`。
- [x] 构建警告仍仅为已知的大字体、大 chunk 和动态/静态导入提示；三个用户未提交文件未触碰、未暂存。
- [ ] 系统 Chrome、reduced-motion、性能、完整失败状态、Figma Canonical 和可读独立 Critic 仍未完成；页面继续 `noindex,follow`。

## 2026-09-12 本地公开主页运行时复核
...
## 2026-09-12 独立 Critic 复验结果

- [ ] 已将最新公开导航修复交回既有 Critic 任务；任务完成但仍返回空 `items`、无可读六维正文，不能计为通过，证据为 `EVD-20260912-025`。
- [ ] 不创建重复审查线程；在 Critic 输出能力恢复前，独立审查保持阻塞，页面继续 `noindex,follow`。

## 2026-09-12 缺图回退版式收口

- [x] 普通鱼类 Species 路由的图片回退媒体高度收紧为稳定的 320px，减少无图状态的大面积空白；已批准物种图片布局不变。
- [x] 新增响应式契约断言，保护缺图回退使用紧凑媒体框；提交 `ea8a96a6`。
...
## 2026-09-12 Species FAQ 键盘回归（`5ce55f95`）

- [x] 内置浏览器通过 Tab 聚焦第一条 FAQ，再按 Space 展开并显示答案，焦点保持在按钮，证据为 `EVD-20260912-075`。
- [ ] 该证据仍不替代系统 Chrome 多视口和完整键盘回归。

## 2026-09-12 Species 品系键盘回归（`b8efa940`）

- [x] 内置浏览器通过 Tab + Enter 进入黄金米虾品系，URL、H1、Alt、18–28°C 参数和黄色差异正确，证据为 `EVD-20260912-076`。
- [ ] 该证据仍不替代系统 Chrome 多视口和完整键盘回归。

## 2026-09-12 Category → Species 键盘回归（`e0fcd76f`）
...
## 2026-09-12 Species → 兼容工具键盘回归（`b8d98fa6`）
...
## 2026-09-12 宝莲灯 → 兼容工具键盘回归（`2d3c2cfc`）

- [x] 宝莲灯通过 Tab + Enter 进入兼容工具，保留 `sp_0432`、来源参数、已选宝莲灯和无缸状态，证据为 `EVD-20260912-085`。
- [ ] 该证据仍不替代系统 Chrome 多视口、性能/reduced-motion、Figma Canonical 或可读独立 Critic。
...
## 2026-09-13 公开收藏失败反馈收口（`1415bcf3`）

- [x] 独立公开收藏写入失败时现在会进入页面已有失败反馈，不再显示错误的“已加入收藏”，证据为 `EVD-20260913-095`。
- [x] 公开结构、契约、lint、build 和 diff-check通过；应用内收藏行为未改变。
## 2026-09-13 公开收藏存储隔离专项（`89d977da`）
...
## 2026-09-13 鱼类与品系继承运行复核

- [x] 宝莲灯本地页面显示已确认的中层活动、群游倾向/至少 5 条、取食和环境章节；未审核的日常、FAQ、品系内容自然省略，证据为 `EVD-20260913-109`。
- [x] 黄金米虾本地页面显示自身 18–28°C、黄色差异，并继承极火虾活动与觅食内容；未生成独立品系生活习性证据，证据为 `EVD-20260913-109`。
- [ ] 仍待三档完整截图、托管 DOM/性能、可读 Critic、Figma Canonical 和索引发布门禁。
## 2026-09-13 宝莲灯工具入口与返回
...
## 2026-09-13 孔雀鱼未审核文案门禁收口 (`5bc61b1b`)
...
- [x] 继续压缩用户端文字：鱼缸能力卡改为“比较它与我的鱼缸是否匹配”，资料区说明缩短为“信息来源”。
- [x] 统一生活习性与取食章节标题：行为章节使用“生活方式 / 它如何生活”，单独取食内容使用“日常 / 取食方式”，移除“已确认”等内部审核口吻。
...
- [ ] 同一 Critic 线程已复验 `bd5356cf`，任务完成但仍无可读正文；按审查基础设施阻塞处理，不创建重复线程。
...
## 2026-09-14 Species 页面章节命名统一（已提交 `2d610a1d`，进行中）
...
- [x] 新增重复章节标题回归门禁，阻止“生活方式 / 它如何生活”“环境 / 适合怎样的环境”及动态切换“取食方式”重新出现；提交 `28014f9e`，`test:public-seo-copy`、lint 和 diff-check 通过。
- [x] 内置浏览器复核宝莲灯普通页面：缺图回退、身份与参数、已确认生活习性/环境章节和工具入口可见，缺失章节自然省略；记录 `EVD-20260914-167`。该视口不替代 390/600/1440 截图。
- [x] 内置浏览器复核黄金米虾与孔雀鱼：黄金米虾继承基础物种生活习性且保留自身参数/黄色差异；孔雀鱼仅显示身份、参数、来源与图片回退，无未经确认正文；记录 `EVD-20260914-168`。
- [x] 尝试在内置浏览器建立精确 390px 视口，接口不提供 `setViewportSize`；当前窗口不计入 390px 证据，记录 `EVD-20260914-169`。
- [x] 远端网络恢复后成功推送 `codex/species-seo-preview-v1`，远端从 `c2f59e6e` 更新到 `d3596494`；记录 `EVD-20260914-170`。
- [ ] GitHub Actions 新运行尚未读取：API 限流且 CLI 连接失败，不能把推送成功当作浏览器验收通过。
