...
- Changed: Species 公开页面的行为、环境、日常章节统一为单一用户标题，移除重复的辅助眉题；“日常怎么养”不再按内容类型切换标题，避免页面信息层级不一致。证据为 `EVD-20260914-165`。
- Added: 公开文案回归测试新增重复章节标题门禁，防止旧的辅助眉题和动态“取食方式”章节标题重新进入用户页面。证据为 `EVD-20260914-166`。
- Verified: 内置浏览器复核宝莲灯普通页面的缺图回退、身份/参数、已确认生活习性与环境章节及能力入口；缺失章节自然省略。证据为 `EVD-20260914-167`。
- Verified: 内置浏览器复核黄金米虾基础物种继承与孔雀鱼缺失正文状态；两者的参数、工具入口、图片回退和公开边界符合门禁。证据为 `EVD-20260914-168`。
- Blocked: 内置浏览器接口不支持直接设置 390px 视口，当前窗口不计入精确移动端验收。证据为 `EVD-20260914-169`。
- Verified: `codex/species-seo-preview-v1` 已成功推送至远端并包含 `d3596494`；GitHub Actions 结果尚待读取，不将推送视为浏览器验收。证据为 `EVD-20260914-170`。

### Changed

- 为 GitHub Actions 浏览器门禁增加失败诊断 artifact：失败时记录错误名称、消息、堆栈和请求日志，便于定位无正文的 CI 失败；不降低断言标准。
- 修复 GitHub Actions FAQ 键盘验收对 React `useId()` 冒号 ID 的定位方式；改用属性选择器，保持原有可见性、无障碍和焦点断言不变。
- 强化公开 SEO 浏览器门禁：交互目标使用真实尺寸判断，FAQ 必须存在且展开后验证答案可见、无障碍状态和焦点保持。
- Species 页面第二轮内容减法：缩短鱼缸能力卡与资料区说明，统一生活方式和取食章节标题，移除“已确认”等内部审核口吻；未改变页面事实、路由、素材和索引策略。

- Species 产品能力入口改为用户问题式表达，并移除相关内容区重复的混养计算入口，减少同一动作的重复文案。
- 收紧 Species 用户端内容：移除无证据时的通用 Hero 话术、环境装饰占位和章节导航中的产品入口；核心参数与已确认取食内容使用更直接的中文标题。

### Changed

- 完成 Species 390/600/1440 对照图视觉审计，新增视觉审计记录；极火虾/黄金米虾保持确认素材，宝莲灯和孔雀鱼继续按素材门禁显示回退。
- 扩展 GitHub Actions 浏览器门禁，增加滚动、章节锚点、浏览器返回、reduced-motion 和交互延迟基线采集；run `34766143025` 已通过，交互延迟本次为 `null`，不计为性能通过。
...
- `14ffac3b` 已交回既有 Critic 同线程复验，但最新 turn 仍返回空正文；记录为独立审查基础设施阻塞，不创建重复线程、不解除 `noindex`。
...
- 为加载骨架的媒体框和文字轮廓补充 `aria-hidden` 回归断言，防止装饰语义回退（`e644e95e`）。
- 验证加载语义回归提交可完成生产构建；仅保留既有构建警告（`e644e95e`）。
- 核对内置 Chrome 运行时侧栏节点，确认其来自第三方扩展而非 AquaGuide Public Shell（证据 `EVD-20260913-127`）。
- 核对既有 Critic 任务终态为 `idle` 且无 assistant 正文，明确记录为独立审查阻塞（证据 `EVD-20260913-128`）。
- 探测 Playwright WebKit/Firefox 作为 600px 替代运行时；两者均未能启动到页面，记录为环境缺口（证据 `EVD-20260913-129`）。
...
- Verified: 2026-09-12 内置浏览器通过 Tab + Enter 切换黄金米虾品系，URL、H1、图片 Alt、参数和黄色差异正确；证据为 `EVD-20260912-076`。
...
- Verified: 2026-09-12 内置浏览器通过 Tab + Enter 从宝莲灯 Species 进入兼容工具，物种 ID、来源参数、已选宝莲灯和无缸状态正确；证据为 `EVD-20260912-085`。
...
- Verified: 2026-09-12 内置浏览器复核宝莲灯、虾螺蟹分类页和新鱼入缸指南公开入口状态；证据为 `EVD-20260912-068`。
- Verified: 2026-09-12 内置浏览器确认宝莲灯鱼缸能力入口保留 `species=sp_0432` 与 `source=species-profile`；证据为 `EVD-20260912-069`。
- Verified: 2026-09-12 内置浏览器确认兼容工具返回宝莲灯公开页后内容与导航恢复；证据为 `EVD-20260912-070`。
- Verified: 2026-09-12 内置浏览器确认宝莲灯首页面包屑可返回品牌首页；鱼类分类文字在分类页尚未建立前保持不可点击；证据为 `EVD-20260912-071`。
- Verified: 2026-09-12 当前提交的公开 SEO 静态发布门禁全套通过；证据为 `EVD-20260912-072`。
...
- Verified: 2026-09-12 内置 Chrome 实际复核黄金米虾品系进入兼容工具并返回，物种 ID、来源参数、继承内容和返回状态正确；证据为 `EVD-20260912-046`。
...
- Added: 2026-09-12 公开文案回归同时保护继承 FAQ 的问题和答案使用当前品系名称，提交 `f8cd8b79`。
...
- Verified: 2026-09-12 章节当前态提交后的公开 SEO 全套静态/构建门禁通过，证据为 `EVD-20260912-034`；未改变内容、素材或索引策略。
- Verified: 2026-09-12 内置 Chrome 首屏截图确认章节当前态视觉高亮已呈现；仅覆盖当前桌面首屏，不替代多视口滚动验证。
- Fixed: 2026-09-12 分类页单个基础物种卡改为舒适单列宽度，多物种才启用双列；提交 `8c045c34`。
...
- Blocked: 2026-09-12 既有 Critic 针对最新公开导航修复的复验任务完成但仍无可读正文；空输出不计为独立审查通过。
...
- Verified: 2026-09-12 内置浏览器完成极火虾、宝莲灯、黄金米虾三条路径 × 390/600/1440 共九次真实加载，确认滚动、2/3/6 列、H1、公开 Header、内容继承和 `noindex,follow`；证据为 `EVD-20260912-008`。
- Verified: 2026-09-12 内置浏览器完成首页、分类页和未发布 Guide 三条路径 × 390/600/1440 共九次真实加载；确认公共壳层、滚动、无溢出、H1、robots 和 JSON-LD 门禁，证据为 `EVD-20260912-009`。
...
- Verified: 内置浏览器确认分类页入口和未审核指南准备态可读；未审核步骤未进入公开内容。
...
- Added: 宝莲灯 Hero/品系卡素材加入独立 SHA-256 与 snapshot fingerprint binding；当前保持 blocked，批准前不会进入 Published Profile。
- Verified: Evidence bindings、TypeScript、production build 与 diff-check 通过；既有字体和大 chunk 警告未扩大。
...
- Verified: 内置浏览器完成分类 → Species → 浏览器返回路径，URL 与页面内容恢复正常。
- Verified: 纯 Vite 预览完成相同分类 → Species → 返回路径；公开 SEO 页面无需 API 进程即可渲染。记录常规开发脚本的 API/tsx 环境阻塞。
...
- Verified: Draft PR #147 的 `foundation`、`validate`、Vercel 和 Cloudflare 检查全绿；本地、远端、PR 与 READY Preview 精确匹配 `01d71f12`。独立 Critic 未返回正文，继续作为发布门禁阻塞。
...
- 2026-09-10：统一公开 SEO 章节标题层级；相同眉题不再与正式标题重复显示，不影响内容或索引策略。
...
## [Unreleased] Verification — 2026-09-12
...
- Chrome 用户标签补充黄金米虾切换与浏览器返回证据：共享生活习性、品系图片 Alt、H1、URL 和 `noindex` 正确，记录为 `EVD-20260912-016`。
- Chrome 用户标签补充 Species 到鱼缸工具的真实跳转证据：物种 ID、来源参数、工具内容和返回百科状态正确，记录为 `EVD-20260912-017`。

- 公开资料区归并同一 FishBase 物种的 URL 变体，保留底层 evidence 绑定与来源追溯。
- 新增契约回归，确保宝莲灯公开 FishBase 来源只展示一次。
...
- In progress: 2026-09-13 已向既有 Critic 派发 `09fd37d3` 的只读六维复验；任务仍 active，尚无报告，证据为 `EVD-20260913-112`。
- Blocked: 2026-09-13 既有只读 Critic 完成本轮复验但仍返回空 `items`，没有可读六维正文；不计为独立审查通过，不创建重复线程，证据为 `EVD-20260913-113`。
- Changed: 2026-09-13 收紧公开 Species 章节响应式节奏，并将环境抽象区改为“水体/空间/稳定”结构化观察面板；提交 `4a7d39e8`，证据为 `EVD-20260913-114`。未改变生物事实、素材状态、路由、metadata、JSON-LD 或 `noindex`。
- Verified: 2026-09-13 内置浏览器本地 `390px` 复核确认 Species 章节、图片、内容和链接均存在；fullPage 重复为截图拼接伪影，证据为 `EVD-20260913-115`。
- Verified: 2026-09-13 内置浏览器约 1451px 视口复核 Species Hero、双列布局、标题层级、无横向溢出和 `noindex,follow`，证据为 `EVD-20260913-116`。
