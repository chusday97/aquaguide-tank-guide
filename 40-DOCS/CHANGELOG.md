...
- 再次补齐逐对签名的全部规则码与方向无关物种 ID，关闭反向选择重复记录边界。
...
- 修复清空本地状态后的 discovery 残留：显式清除会删除 legacy 镜像并通知页面，Aquarium 不再保留旧队列；普通保存继续保留旧 key 以兼容尚未迁移用户。
...
- 新增 P0 兼容性/当前鱼缸契约草案（历史记录，已由用户批准并在 `99865414` 中按本地-only 范围实现）：仅涉及本地生命周期、证据与派生服务，不新增 SQL、API、Supabase 持久化或 UI 几何。
- 新增统一发布就绪表，分别记录本地 UI、CI、部署 SHA、Supabase parity、P0 契约与 main release 许可，禁止以单一绿灯宣称可发布。
- 建立跨层模块事实盘点，统一记录产品承诺、UI 入口、领域规则、Service、数据/API、测试、部署状态和当前模块状态。
- 关闭除 #141 外的 55 个历史 GitHub PR，保留远端分支和可追溯记录；当前只允许 #141 作为收敛入口。

### Fixed
- 修复图鉴与养护桌面详情曾因脱离 Base UI Portal 而白屏、并且养护详情没有真实参与父级双栏网格的问题；现以页面内工作区区域重排，运行时门禁同时检查非固定、非 Portal、无滚动锁和左右不重叠。
- 桌面物种与养护详情切换为页面网格内的真实双屏表面，避免固定 Portal 覆盖浏览区；首页 3D 鱼缸新增 `stage-cover` 响应式取景。
- 互动图鉴“换一批”改为完整替换当前六个物种：同日批次不重复、刷新保留当前批次、全部浏览后需用户明确重新开始；旧单项推荐队列保持兼容。
- 场景物种的加载态、失败态和容器不再生成矩形图片底板；场景资源由 Alpha 通道与透明角像素门禁验证。
- 互动养护六个热点初始即显示类型名称；症状只在悬停、键盘聚焦或点选后展开，手机不再隐藏名称。

### Changed
- 修正桌面双屏详情的实现边界：Base UI `Dialog.Popup` 恢复仅在 Portal 中使用，工作区详情改为页面内 `section`；养护详情由真实 shell 网格重排，不再以固定抽屉覆盖浏览区。
- `stage-cover` 相机改为真正覆盖鱼缸内部的取景距离，允许裁切外框；新增 1440px 浏览器双屏几何门禁与多宽高比取景门禁。
- 部署端 AI 健康与聊天处理器进一步对齐：有效的 DeepSeek 回退密钥可正确生效；建缸助手与每日检查在 Vercel、Pages 均覆盖成功、未配置、网络和非法 JSON 的一致失败语义。
- Vercel 与 Pages 健康检查统一排除示例占位密钥；两端 AI handler 都拒绝空/数组结构化回复并返回明确失败原因，避免把本地兜底误标为模型成功。
- Vercel AI handler 与本地 Express 对齐建缸助手和每日检查的结构化任务协议，并统一返回模型来源、生成时间和失败原因；页面可区分模型回复与本地安全兜底。
- 养护百科显式拆分“互动排查 / 传统浏览”：默认 `/care` 只呈现场景化排查，搜索、分类与文章列表在 `/care?mode=browse` 中按需打开；两种模式继续共享文章详情、收藏和返回上下文。
- 互动图鉴与场景化养护统一改用物种透明贴图，选中其他对象时以透明度而非模糊滤镜区分；透明加载与失败状态不再生成浅灰或白色图片底块。
- 鱼缸首页工作区不再受通用 1280px 画布限制；桌面 3D 舞台铺满侧栏以外的工作区，保留顶部控制和场景 HUD。

### Added
- 新增 P0 当前鱼缸与换水确定性测试（11 个 tank-state、10 个 tank-evidence、8 个 water-change 场景），并将其加入项目脚本；临时 4320 生产预览的布局、3D 取景、互动场景和页面运行矩阵通过。
...
- 健康接口新增独立的文字 AI 与视觉 AI 能力状态；开发启动脚本会先等待 API 健康检查成功，再启动网页预览。
- 今日推荐从鱼缸首页迁入互动图鉴：场景中的“换一批”直接推进既有当日十项候选队列，用户在探索鱼缸中点选感兴趣的生物后再进入档案；首页不再出现重复推荐卡。
- 物种与养护详情的桌面表面升级为双屏协作工作区：右侧占据半屏阅读与操作，左侧图鉴/养护场景仍可浏览与切换对象；不再添加遮罩、背景模糊或页面滚动锁定，真实手机继续使用底部详情面板。
- 水族册首页从静态四格升级为可探索的水下书册：种草、养护、纪念与勋章分别以实体章节呈现，点击执行原位翻页；返回总览恢复原章节焦点，具体条目与“更多”继续复用既有深链和数据源。
- 鱼缸首页改为连续 3D 主舞台：首页自动使用图标侧栏，今日行动悬浮于鱼缸右上，六项高频任务进入底部操作坞；独立今日推荐和鱼缸档案不再占据首页。普通模式为当前状态，“怀孕模式 · 规划中”为只读提示。
- 水族册首页改为“水下展开书册”：四个章节在同一观察册中展示真实最近内容，点击章节原位展开，具体条目与“更多”继续使用原有深链，不新增弹窗或第二套收藏数据。
- 用户端排版收口为自然杂志体系：展示标题使用宋体/Georgia 回退，功能文字使用 Geist 与中文系统无衬线回退；科学名称保留斜体，中文标题不再被强制斜体。
- 鱼缸首页桌面端改为沉浸式 3D 主舞台：保留现有 `ThreeAquarium`，右侧收口为今日行动与今日推荐，全部高频任务改为主画面内的底部快捷栏；不再向首页插入独立的静态或探索鱼缸。
- 首页顶部增加只读的普通模式标识与“怀孕模式 · 规划中”提示，避免把尚未实现的模式伪装成可切换功能。
- 桌面内容详情统一改为右侧阅读面板，任务流程改为右侧任务面板；手机继续使用底部详情与全高任务表面。
- 物种与养护详情的主 CTA 从底部上移到结论旁，收藏、下载和分享移入内容工具区；缸内体态调整改用更轻的编辑式布局。
...
### Added
- 新增 AI Badcase 最新状态记录：本轮确定性与模拟 Provider 评测共 47/47 通过，未伪造失败记录；UI 视觉待验证项继续保留在产品交接与进度文档中。
- 新增互动场景第一版：图鉴与养护默认以大型鱼缸呈现，物种图片在稳定热区内游动并可点选；图鉴选择后显示现有物种档案入口，养护按水面、水体、生物、过滤、底床或水草设备进入既有养护指南。首页不承载新增场景鱼缸，保留既有行动工作区；所有入口仍复用既有文章、详情、诊断和写入路径。
