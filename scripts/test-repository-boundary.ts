...
assert.doesNotMatch(aquariumPageSource, /repositoryAquariums\.length === 0\) return/);
assert.match(aquariumPageSource, /if \(!active \|\| mode === 'cloud'\) return/);
assert.match(aquariumPageSource, /setActiveId\(current => normalized\.some[\s\S]{0,160}normalized\[0\]\?\.id \|\| ''/);
assert.match(aquariumPageSource, /生物已经记录；时间线暂未更新/);
assert.match(encyclopediaPageSource, /recorded\.savedItems\.forEach\(item => next\.add\(item\.fishId\)\)/);
