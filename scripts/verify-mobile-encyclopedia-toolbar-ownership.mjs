...
try {
  ...
  assert.equal(topControls, 6, 'Encyclopedia toolbar should expose Scene, Browse, Compatibility plus Search, Photo ID, and Settings');
  const modeButtons = toolbar.locator('div.grid button');
  for (let index = 0; index < 3; index += 1) {
    const box = await modeButtons.nth(index).boundingBox();
    assert.ok(box && box.width >= 58, `mode button ${index} must stay readable at 390px`);
  }
