...
assert.ok(projection.route.blockers.some(item => item.includes('hosted acceptance')));
...
const editorialApi = readFileSync(resolve(root, 'apps/api/src/care-seo-editorial.ts'), 'utf8');
...
assert.match(editorialApi, /from\('content_publications'\)/);
assert.match(editorialApi, /source_version/);
assert.match(editorialApi, /publication-snapshot/);
assert.match(editorialApi, /eq\('status', 'published'\)/);
assert.match(editorialApi, /legacy-published/);
