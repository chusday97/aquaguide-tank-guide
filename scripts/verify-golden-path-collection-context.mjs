...
const assertFocusCarousel = async page => {
  ...
  assert.equal(await cards.count(), 3, 'Collection hub primary carousel must expose only the three live modules.');
  ...
  assert.equal(await comingSoon.getAttribute('data-feature-status'), 'building', 'Achievements must remain an explicit building surface outside the primary carousel.');
  assert.equal(await comingSoon.locator('button').count(), 0, 'Building achievements must not expose a fake business CTA.');
  ...
  assert.equal(visibleSideCards, 2, `Three-module carousel must visibly preview both live neighboring modules; got ${visibleSideCards}.`);
  ...
  assert.equal(await indicators.count(), 3, 'Collection carousel must expose one position control per live module.');
...
try {
  ...
  console.log('GP-005 continuous E2E passed: live focus carousel → Wishlist horizontal context → exact saved species → desktop drawer/mobile sheet → close → exact card context and rail position preserved.');
} finally {
  await browser.close();
}
