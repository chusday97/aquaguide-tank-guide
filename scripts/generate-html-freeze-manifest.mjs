...
const manifest = { schemaVersion: 1, freezeVersion: 'html-ui-freeze-v3', generatedAt: new Date().toISOString(), branch: git(['branch', '--show-current'], 'unknown'), gitSha: git(['rev-parse', 'HEAD'], 'unknown'), references: { aquarium: '4196', collection: '4196', encyclopedia: '4197', care: '4197', compatibility: '4197' }, requiredViewports: [390, 600, 1024, 1440, 1920], files: files.sort().map((path) => ({ path: relative(root, path), bytes: statSync(path).size, sha256: digest(path) })), pageChecks, acceptance: { status: 'REVIEW_REQUIRED', blocks: ['aquarium-stage', 'collection-overview', 'encyclopedia-flow', 'care-flow', 'compatibility-board'] } };
