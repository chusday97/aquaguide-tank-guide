...
assert.deepEqual(caution.verdict,{status:'caution',label:'调整后可尝试',indicator:'yellow'}); assert.equal(caution.reasons.length,1); assert.match(caution.reasons[0],/最低群体数量/); assert.deepEqual(caution.adjustments,['先补足已审核的最低群体数量，并重新核对增加数量后的空间与负荷。']);
