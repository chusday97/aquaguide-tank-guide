...
begin
  ...
  from public.idempotency_records as operation
  where operation.owner_id = current_user_id
    and operation.idempotency_key = operation_key
  ...
  if found then
    ...
    into latest_at
    from public.care_events as event
    where event.owner_id = current_user_id
      and event.aquarium_id = target_aquarium_id
      and event.event_type = 'water_change'
      and event.source_type = 'water_change_day'
  ...
  select *
  into current_aquarium
  from public.aquariums as aquarium
  where aquarium.id = target_aquarium_id
  ...
  if should_record then
    if not exists (
      ...
      where event.owner_id = current_user_id
        ...
        and event.event_type = 'water_change'
        and event.source_type = 'water_change_day'
        and event.source_id = water_change_date::text
        and event.deleted_at is null
    ) then
    ...
    update public.care_events as event
    set deleted_at = now()
    where event.owner_id = current_user_id
      and event.aquarium_id = target_aquarium_id
      and event.event_type = 'water_change'
  ...
  update public.aquariums as aquarium
  set last_water_change_at = latest_at
  where aquarium.id = target_aquarium_id
    and aquarium.owner_id = current_user_id
    and aquarium.deleted_at is null;

  update public.aquarium_species as species_record
  set last_water_change_at = latest_at
  where species_record.aquarium_id = target_aquarium_id
