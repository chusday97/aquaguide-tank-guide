-- Add the reviewed Mikrogeophagus ramirezi (Ram) Compatibility profile without mutating prior baseline migrations.
...
begin
  ...
  from public.species where catalog_key='sp_0448' and deleted_at is null;
  if existing_count=0 then
    insert into pg_temp.compatibility_ramirezi_baseline_gate(mode) values ('skip');
    raise notice 'Compatibility ramirezi baseline absent; skipping reviewed-data extension.';
  elsif existing_count=1 and published_count=1 then
    insert into pg_temp.compatibility_ramirezi_baseline_gate(mode) values ('run');
  else
    raise exception 'Compatibility ramirezi baseline is partial or not fully published: existing %, published %, required 1', existing_count,published_count;
  end if;
end $$;
...
from public.species s where s.catalog_key='sp_0448' and s.deleted_at is null and s.status='published'
  and exists (select 1 from pg_temp.compatibility_ramirezi_baseline_gate where mode='run')
...
select cp.id,e.id from public.species_compatibility_profiles cp join public.species s on s.id=cp.species_id join public.evidence_sources e on e.source_key='seriouslyfish-mikrogeophagus-ramirezi'
where s.catalog_key='sp_0448' and exists (select 1 from pg_temp.compatibility_ramirezi_baseline_gate where mode='run') on conflict do nothing;
insert into public.species_compatibility_profile_sources(profile_id,source_id)
select cp.id,e.id from public.species_compatibility_profiles cp join public.species s on s.id=cp.species_id join public.evidence_sources e on e.source_key='fishbase-mikrogeophagus-ramirezi'
where s.catalog_key='sp_0448' and exists (select 1 from pg_temp.compatibility_ramirezi_baseline_gate where mode='run') on conflict do nothing;

do $$ begin
  if exists (select 1 from pg_temp.compatibility_ramirezi_baseline_gate where mode='run') then
    if not exists (
      select 1 from public.species_compatibility_profiles cp join public.species s on s.id=cp.species_id
      where s.catalog_key='sp_0448' and cp.behavior_traits=ARRAY['peaceful','breeding_defense']::text[] and cp.minimum_group_size is null
    ...
    ) then raise exception 'Compatibility ramirezi profile drift: sp_0448'; end if;
    if (
      ...
      where s.catalog_key='sp_0448'
    ) <> ARRAY['fishbase-mikrogeophagus-ramirezi','seriouslyfish-mikrogeophagus-ramirezi']::text[]
    then raise exception 'Compatibility ramirezi profile evidence drift: sp_0448'; end if;
  end if;
end $$;

drop table pg_temp.compatibility_ramirezi_baseline_gate;
