...
- **Preview parity（实时复核）：** Vercel READY Preview `aquaguide-60xddgoyg-chusday97s-projects.vercel.app` 的 deployment SHA 与本地/远端 `b2613bdd` 完全一致，`npm run check:preview-parity` 返回 `PASS`。Supabase schema/RLS parity 和用户 release acceptance 仍未完成。
