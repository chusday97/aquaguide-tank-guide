...
- 分支：`codex/catalog-cohort-30-v1`，基于 `main@016dbca5`；代码提交 `9b3b9380`，当前文档 HEAD `6bb79515`，本地后续修复尚未提交/推送。
...
- Critic 首轮发现的引用绕过和行为字段丢失问题正在修复：审核脚本现在要求 citationId 指向同一物种的已审核 source，并拒绝无效 reviewed 值；领地、捕食和繁殖字段会保留到 Profile 的 `factEvidence`，避免审核通过后静默丢弃。
