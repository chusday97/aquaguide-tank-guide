...
- 4317 继续是 detached `37a8d4d1` 视觉母版；候选 4319 当前运行提交 `d71ab59d` 的 production preview，不能用 Vite 开发态的白色 Canvas 作为验收证据。
- 已修复：Aquarium 预览工作区的明确高度、4317 字体变量/标题层级、截图 ready/font/Canvas 等待和 4xx/pageerror 失败门禁。
- 已验证：候选 production build 在 4319 渲染 WebGL 鱼缸、透明生物和操作层；无页面错误。最新固定截图位于 `/private/tmp/aquaguide-visual-matrix/ui-parity-d71ab59d`。
