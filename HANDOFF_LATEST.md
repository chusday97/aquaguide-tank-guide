...
- 分支：`codex/catalog-cohort-30-v1`，基于 `main@016dbca5`；最新修复提交 `62539910`（前置代码提交 `9b3b9380`），当前仅本地，未推送。
...
- Critic 首轮发现的两个阻塞已在 `62539910` 修复：审核脚本要求 citationId 指向同一物种的已审核 source，并拒绝无效 reviewed 值；领地、捕食和繁殖字段会保留到 Profile 的 `factEvidence`，避免审核通过后静默丢弃。新增测试覆盖无效引用、无效行为值和行为字段 overlay；同一 Critic 复验仍待完成。
