...
## 2026-08-24 — Recommendation authority convergence (#134 / #135)

- #134 found a pre-evaluation authority bypass in both ordinary and Smart Recommendation: static `housingMode = 建议单养` removed a candidate before canonical Compatibility could classify it.
- Repro: 120L freshwater tank with existing `sp_0431` 红绿灯 and candidate `sp_0016` 金波子. Canonical Compatibility returned `insufficient_data`, `blockingRules=[]`, warning-only `hard_species / single_housing`, plus missing evidence; Recommendation nevertheless returned no candidate.
- #134 removed only the two housing-mode prefilters and added `scripts/test-recommendation-authority.ts` to the permanent P0 Compatibility gate. Candidate `1f7b77326453bc824bb9b179d6914883b3fefb93` passed 16/16 triggered workflows; merged runtime `dfa095f3ba65ac3cfb115944b42ea03dc5f0be42` passed 16/16 exact-head checks.
- #135 then reproduced three post-evaluation severity escalations: heuristic near-limit load, reviewed min-group capacity gap, and the Smart Recommendation `loadBlocked` post-filter could turn canonical `caution / insufficient_data` with `blockingRules=[]` into `blocked` or clear the direct/adjustable pool.
- Fail-before `7d6e85c032471d8dfb1ea0bba0d876db2da04a7d` extended the same Recommendation authority contract with concrete near-limit, min-group, and 95% heuristic-load cases.
- Implementation `7a85b5c048b08f01c0ea6d9ee8d068220e2b844c` keeps load/min-group signals in risks/required adjustments but removes their hard-block authority; true `blocked` remains owned by canonical `compatibility.status === not_recommended`.
- #135 candidate passed 11/11 triggered workflows, including P0 Compatibility `32719051310`, Whole-Tank `32719051198`, Result UX `32719051212`, Golden V3 `32719051169`, Visual QA `32719051197`, and Interactive Atlas `32719051287`.
- #135 merged normally to RC1 as `b69c3c3e3a34f846edf6b59b3007de2ef9f85ad5`. Exact merged RC1 result: **16/16 PASS**. P0: Compatibility `32719324560`, Tank State `32719324505`, Tank Evidence `32719324495`, Existing Tank `32719324538`, Water Change `32719324544`, Whole-Tank `32719324512`. Release/UI: RC1 Release `32719324574`, Product Golden `32719324491`, UI Interaction `32719324513`, UI UX System `32719324624`, UI UX Visual QA `32719324529`, UI UX Golden V3 `32719324585`, UI V2 Aquarium `32719324578`, Navigation `32719324461`, Bundle `32719324479`, Result UX `32719324489`.
- `AQ-BC-REC-001` and `AQ-BC-REC-002` are regression-verified. No Product Truth, canonical Compatibility, Whole-Tank engine, RC1→main merge, or production deployment change was made by #134/#135.
- Recommendation authority audit exit: no currently reproduced hard-block bypass remains in the audited live path. Further Recommendation changes require a new executable canonical-vs-consumer contradiction; next phase is Production Acceptance.
