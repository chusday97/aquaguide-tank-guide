...
> 写给一个完全没有此前对话上下文的新接手者。最后更新：2026-07-26（Asia/Shanghai）。

## 项目一句话说明

- 项目解决什么问题：帮助水族新手管理鱼缸、选择物种、判断混养并完成养护补救。
- 目标用户：刚开始养鱼或缺少系统养护经验的用户。
- 当前阶段：首页 C、缸内物种安全移出、风险向导、pH 非阻断、侧栏切缸、重命名和设置反馈均已完成；Critic 首轮问题已修复，等待同线程复验与 Evaluator 裁决。
...
- 全量 `npm run lint` 当前被此前未完成的英文翻译改动阻断（多处缺失 `isEn` / `i18n` 等符号）；本轮不修改英文，先使用专项测试和构建区分本轮回归，最终仍需单独清理该基线。
- 本轮实现和专项回归已完成；待同一 Critic 复验与 Evaluator 独立验收。
...
1. 将审查修复提交和验证证据交回同一 Critic 复验。
2. Critic 通过后由 Evaluator 对照首页、风险、移出、切缸、重命名和反馈路径裁决。
