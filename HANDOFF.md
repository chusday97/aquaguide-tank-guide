...
- 成功显示“已收录到水族册 / 已从水族册移除”，失败回滚心形状态并展示原因。
...
- 41 篇文章均通过本地来源注册表显示动作支持范围和审核状态；35 篇已审核，6 篇草缸/海缸专项细节明确标为待复核。
- 安全换水不再依赖静置 24 小时处理氯胺；死鱼处理不再自动建议药浴和主缸杀菌。
- 验证：`audit:care-evidence`、`test:care-guidance`、`test:care-guide-types`、`test:care-categories`、`lint`、`build`、`git diff --check`。
