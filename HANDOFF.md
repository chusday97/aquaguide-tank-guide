...
- 因此没有对 Public Shell 做误修；公开页面仍不渲染应用侧栏。
...
- 本轮没有改变公开内容、数据契约、路由、metadata、JSON-LD 或 `noindex,follow`。
- 600px 独立视口、可读 Critic、Figma Canonical、性能/reduced-motion 和索引发布门禁仍保持未完成。
...
- 内置浏览器从极火虾进入黄金米虾后，确认品系身份、黄色图片 Alt、18–28°C、自身工具参数和基础物种行为/觅食继承均正确，证据为 `EVD-20260913-097`。
- 当前 Base/Variant 运行时边界未因收藏服务隔离而回归；完整多视口和独立审查仍未完成。
## 2026-09-13 托管 Preview 版本阻塞

- 已登录 Chrome 能打开托管 Preview，但宝莲灯页面明显不是当前工作树：仍有旧的 `/encyclopedia` 面包屑，缺少当前日常/品系/FAQ结构，相关分类链接也仍指向 App 路由，证据为 `EVD-20260913-098`。
- 因此托管页面暂不能用于发布验收。下一步先核对本地 HEAD、远端分支、Draft PR 和部署 SHA；未完成 parity 前不推断托管页面代表当前实现。
## 2026-09-13 Preview SHA parity 阻塞

- 本地 `HEAD=e8e3029b`，现有远端跟踪引用为 `93f199c1`，本地领先 204 个提交；实时 `git ls-remote` 因 DNS 无法解析 GitHub 失败，证据为 `EVD-20260913-099`。
- 托管页面已确认是旧结构，因此暂不能作为当前代码验收依据。网络恢复后先读取远端/部署 SHA；本轮没有推送或生产变更。
## 2026-09-13 最新交接

- 当前 HEAD：`b0bffe59`；分支：`codex/species-seo-preview-v1`。
...
- 内置浏览器实际复核黄金米虾品系页，确认继承内容、黄色差异、自身参数、FAQ、Alt、工具链接和资料来源均正常，证据为 `EVD-20260913-104`。
## 2026-09-13 Preview 部署侧验证
...
- 本地服务在 `3001` 启动；极火虾 Species 长页面完整可读，章节 `#behavior` 可达，证据 `EVD-20260913-108`。
- 本地运行只验证当前页面结构与锚点，不替代多视口、托管 DOM、性能或独立 Critic。
## 2026-09-13 鱼类与品系复核

- 宝莲灯与黄金米虾均已在本地内置浏览器读取；宝莲灯只展示已确认章节，黄金米虾继承基础物种生活习性且保留自身参数/黄色差异，证据 `EVD-20260913-109`。
- 当前没有新增生物事实、Product Truth 或独立品系 evidence。
## 2026-09-13 宝莲灯工具链路
...
- 内置浏览器在本地 `390px` 真实读取到完整 Species 内容；辅助树显示每个章节只出现一次。`fullPage` 拼接图中的重复区块是截图工具伪影，不作为页面缺陷证据。证据：`EVD-20260913-115`。
- 当前视觉收口代码保持不变；下一次视觉验证使用分段视口截图或可读浏览器 DOM，不再依据单张 fullPage 拼接图判断重复。
## 2026-09-13 桌面视口证据

- 本地内置浏览器在约 1451px 视口完成 Species 运行时复核：Hero 图片、双列布局、章节导航、内容层级和 `noindex,follow` 正常；`h1=1` 且无页面级横向溢出。证据：`EVD-20260913-116`。
- 首次进入的短暂“正在加载 AquaGuide…”由现有加载态覆盖，等待懒加载完成后页面正常呈现；不记录为缺图或空状态缺陷。
- 600px 平板仍是开放验收项。
## 2026-09-13 Critic 复验结论

- 最新 HEAD `b2de6ab0` 已由同一只读 Critic 完成复验，但返回 `items: []`，没有可读六维正文，证据为 `EVD-20260913-120`。
- 该工具结果不计为独立审查通过；当前保持 `noindex,follow`，不创建重复 Critic。
## 2026-09-13 600px 环境探测

- 内置 Chrome 页面脚本不能调用 `window.resizeTo`，因此当前标签页无法被脚本调整到 600px；页面未被修改，证据为 `EVD-20260913-121`。
- 600px 仍是未完成的真实视口证据，不把响应式静态契约当成运行时通过。
## 2026-09-13 宝莲灯素材预览
