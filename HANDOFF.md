...
- 56 个 open PR 已归档到 `.ai/OPEN_PR_REGISTRY.md`：只有 #141 可作为当前收敛入口；其余 55 个只可提供经验证的历史事实，不能直接 merge。
- 下一步：按 `.ai/P0_MIGRATION_IMPACT.md` 完成契约范围确认后，才迁移 P0 业务代码；不可直接 merge/rebase RC 或 main。
- 状态校正：用户确认既有 Supabase 工作已部署。后文历史记录里的“待真实 Supabase 验证”仅指未在当前统一分支重新验证具体环境/schema/RLS，不得解读为未部署。
...
- 内容基于当前 `PROGRESS.md` 与本文件已有事实；未修改产品代码、业务数据或远端状态。
- 下一步仍是正式 React 内部预览、五档视口视觉回归和用户视觉确认；本次未推送远端。本地提交为 `106cf83`。
...
- 当前验证：`npm run lint`、`npm run test:interactive-discovery`、`npm run test:interactive-scene-assets`、`npm run build` 与生产预览 `test:interactive-scenes-ui` 已通过。Critic 六维审查及复验 PASS；其建议的“刷新保留当前批次、透明图双次失败仍无矩形底板”已补入浏览器测试并通过。独立 Evaluator 最终 PASS，无阻塞项。功能分支已推送；本轮仍不自动合并 main。
...
- `4c54f74` 关闭 Critic 的部署一致性阻塞：Vercel/Pages 均忽略占位密钥但识别有效回退密钥；两端 handler 对成功、未配置、网络与非法 JSON 统一返回 `source / generatedAt / failureReason`。`test:ai-capabilities` 直接模拟调用双方 handler，`lint`、API 类型、build、手机养护与 3D 素材测试通过；同线程 Critic 与独立 Evaluator 均 PASS，`adf3082` 已推送。真实托管环境的 Secret 与网络是唯一外部验收项。
...
- 功能提交：`e11cd0e feat(collection): add interactive underwater journal`。同线程 Critic 复验与独立 Evaluator 均 PASS；本次不自动推送远端，静态 HTML 预览不在业务仓库内。
...
- `AquariumWorkspace` 已改为场景、右侧栏、底部任务栏和缸内物种摘要的单一结构。原 3D 点击、生物选择、全屏、设置、时间线、推荐详情与数据读写均未替换。
- 桌面 1440px 与 iPhone 390px 使用种子鱼缸数据验证 `scrollWidth` 无溢出；构建仅保留既有大包警告。Critic 首轮发现移动端旧参数浮层会遮挡主标题、底部档案与舞台会重复打开物种列表；已移除旧浮层、档案改为只读摘要、舞台入口补齐 dialog 语义与专项断言。同线程 Critic 复验及 Evaluator 均 PASS。
...
## 2026-08-21 互动视觉一致性：水族册阶段

- 已完成：`CollectionHub` 从四格静态预览改为书册状态机（总览 → 翻页 → 章节 → 返回）；章节保留原模块深链、最近内容和空状态入口。CSS 增加水下光纹、植物、环境生物与减少动态效果兜底。
