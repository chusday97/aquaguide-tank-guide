...
> 写给一个完全没有此前对话上下文的新接手者。最后更新：2026-07-27（Asia/Shanghai）。
...
- 当前实施：全局交互显性化与折叠治理。阶段 1–5 已完成共享表面标题栏、物种档案直显、自查范围、首页紧凑直显及折叠用途白名单；下一步执行全量回归、独立 Critic 复验与 Evaluator 验收。
...
- 自动门禁：`npm run test:disclosures` 扫描正式折叠，任何未登记用途、喂养摘要、安全动作、首页整区折叠或旧 `+N` 标签都会失败。
