...
- 当前候选没有 exact Preview 部署，`check-preview-parity` 为 `UNVERIFIED`；AquaGuide 与 `admin-content` Vercel checks 均因 24 小时部署限流失败；截至最近复核，Cloudflare pending、foundation 一项通过/一项 pending、`validate` pending。
