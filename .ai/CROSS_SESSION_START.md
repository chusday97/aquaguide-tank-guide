...
Current visual acceptance rule: the CMS must use only Graphite / White / one Green accent, with no separate warning/review/success hues; typography must preserve a clear hierarchy. The publishing workflow is permanent compact top chrome (no expand/focus mode) and must read as navigation/status, clearly smaller than editor typography. Preview is explicit-on-demand from the sticky editor bar and opens as an overlay drawer rather than occupying a permanent third column.
