...
The dedicated feature ↔ live-main reconciliation audit now has an isolated validated candidate. Draft PR #144 carries the candidate; the original two-parent merge checkpoint is `6e5d9aab` (feature `3dfa76af` + live main `64fa58a`). Local build/domain/runtime/Admin/browser gates passed, GitHub light CI passed, and AquaGuide/Admin Preview deployments were established without touching Production.

The first unfinished gate is **human visual acceptance of the reconciled Preview**, followed by a deliberate merge decision. Re-read the current PR HEAD/Preview parity because the parity-resolver/docs commit is layered after `6e5d9aab`. Do **not** auto-merge `main`, unlock index/Production, or apply live migrations. Continue from `TASK_QUEUE.md`; do not reopen completed Product/Care, Compatibility, Publish Center, Species SEO, Care SEO or AI-advisory work unless a new regression is proven.
...
`继续 Aqua 项目。不要根据聊天记忆猜测。先读取 /Users/chuchu/aquaguide-admin-content-v0/.ai/CROSS_SESSION_START.md，再按其中顺序读取 HANDOFF_LATEST / AQUA_OPERATIONS_STUDIO_ARCHITECTURE / CURRENT_GOAL / TASK_QUEUE / LIVE_STATUS / BRANCH_STATUS / EXECUTION_LOG 最新记录。真实核对当前 branch、git status、HEAD、Draft PR #144 和 live remote refs。feature ↔ live-main reconciliation audit 已完成，不要重做；继续 TASK_QUEUE 中第一个未完成项：先确认当前 PR HEAD 的 Preview parity，再对 reconciliation Preview 做人工视觉验收/UI freeze 决策。未经明确授权不要 merge main、不要解锁 Production/index、不要应用 live migration。不要重新实现已完成的 Product/Care、Compatibility、Publish Center、Species SEO、Care SEO 或 AI advisory；Care SEO 继续 hold_noindex；每完成一轮同步 .ai 文档。`
