...
All defined P0 work, P1, P2 Publish Center V1, and the Care SEO projection/static-handoff foundation are closed in code. Care SEO now binds only to Published Care versions, uses deterministic EN/ZH `.html` canonical + hreflang routes, keeps SPA fallback `noindex,follow`, and has a fail-closed static Staging artifact builder. The first unfinished item is **Care SEO Editorial Draft/Review + explicit Staging snapshot/handoff and hosted acceptance**. Do not centralize writes, duplicate Care authority, apply live migrations, or unlock Production.
...
`继续 Aqua 项目。不要根据聊天记忆猜测。先读取 /Users/chuchu/aquaguide-admin-content-v0/.ai/CROSS_SESSION_START.md，再按其中顺序读取 HANDOFF_LATEST / AQUA_OPERATIONS_STUDIO_ARCHITECTURE / CURRENT_GOAL / TASK_QUEUE / LIVE_STATUS / BRANCH_STATUS / EXECUTION_LOG 最新记录。真实核对当前 branch、git status、HEAD 和 live remote refs，然后继续 TASK_QUEUE 中第一个未完成项（当前为 Care SEO Editorial Draft/Review + explicit Staging snapshot/handoff and hosted acceptance）。不要重新实现已完成的 Product/Care、Compatibility、Publish Center 或 Care SEO projection/static-handoff foundation；不要 merge main；不要动 Production；每完成一轮同步 .ai 文档。`
