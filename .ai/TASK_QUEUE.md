...
- [x] 补齐混养逐对可追溯事件：兼容性计算记录规范化 pair status/key，会话级 allowlist 拒绝自由文本与自配对；不改变运行时规则或数据契约，专项回归通过。
...
- [x] 真实验证 RC Convergence 手动复验：run `32990488781` 在 PR 当前 head `33e8bbfe` 成功，旧 stale-head 失败已与当前结果区分。
...
- [x] 对齐物种详情已拥有状态的当前英文 CTA：`Livestock in Tank`，并将 `npm run test:species-detail-ui` 纳入 `UI Regression V1`。
...
- [ ] 完成授权 Supabase schema/RLS 只读 parity；Vercel exact Preview SHA parity 已在 `f465cb76…` 通过只读门禁，当前仍等待授权 schema/RLS 证据。
- [x] 新增 `npm run check:preview-parity`，自动比较 local/remote/Vercel branch Preview SHA；当前 `fb90c07a` 结果为 `PASS`，不把旧 Preview 误报为通过。
