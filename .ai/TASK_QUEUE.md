...
- [x] 公开 Species 节奏收口：章节间距改为响应式压缩范围，环境区增加“水体/空间/稳定”结构化观察标签，避免大面积空白；提交 `4a7d39e8`，静态响应式、结构、契约、lint、build 和 diff-check 通过。
- [ ] 继续收集内置浏览器三档视觉证据；不把本轮 CSS/结构收口当作完整浏览器验收，Critic、Figma Canonical 和发布门禁仍未完成。
## 2026-09-13 已完成：品系展示与孔雀鱼案例

- [x] 品系媒体固定 4:3、`contain`、独立说明区与 390/600/1440 响应式规则（`2e8eea82`）。
- [x] 新增品系卡片契约测试和孔雀鱼 `sp_0436` 基础案例验证。
- [ ] 继续等待 600px 真实浏览器、可读 Critic、Figma Canonical、性能和发布门禁；页面保持 `noindex,follow`。
