...
- 动作：检查 Species 面包屑和相关分类入口的目标路由，移除鱼类到应用内图鉴的公开误导路径。
...
- 在内置 Chrome 打开 `/species/sp_0432`，等待本地预览稳定并读取页面结构与长截图。
- 结果：资料来源区只显示一条 FishBase 记录；页面可滚动，公开内容、工具入口和 `noindex` 正常。
- 证据：`EVD-20260912-028`。

## 2026-09-12 虾类继承运行时复核

- 在内置 Chrome 打开极火虾基础页和黄金米虾品系页并读取完整 DOM。
- 结果：极火虾已批准图片与确认内容可见；黄金米虾显示黄色 Hero、18–28°C、自身差异并共享基础物种生活习性，未创建重复 evidence。
- 证据：`EVD-20260912-029`。
