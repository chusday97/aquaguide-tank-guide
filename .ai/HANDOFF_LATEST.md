...
- [x] 响应式测试已同步 45px 控件令牌，改为数值断言最小值不低于44px，不降低真实浏览器门禁。

## 2026-09-14 44px 交互目标修复

- [x] CI 精确诊断确认失败元素为 Species 收藏按钮，实际高度 `43.999969px`；`--seo-control-min` 已从 44px 调整为 45px，保持视觉语言不变并增加安全余量。
- [ ] 等待新 SHA 的 GitHub Actions 浏览器门禁验证；若通过，再记录 artifact 和回交 Critic。
...
- 内置浏览器 `fullPage` 截图出现疑似滚动拼接重复，但 AX 树只有一份各章节；不要把该工具伪影当作 React 重复渲染。需要独立截图工具或 GitHub artifact 做像素级确认。
- CI 浏览器脚本现在汇总所有公开路由和三个视口的 LCP/CLS/交互采样；本地脚本语法、公开契约、lint、build、diff-check 通过。性能数据仍需下一次 GitHub Actions 运行后读取，当前不设硬阈值。
- 该性能采样改进已提交为本地 `fe426bd6`，推送因 GitHub DNS 暂时失败；远端仍停留在上一个已通过浏览器门禁的 `bd5356cf`。

## 2026-09-14 用户文案与章节层级收口

- Species 页面行为、环境、日常三个章节已移除重复辅助眉题，标题不再出现“生活方式 / 它如何生活”“环境 / 适合怎样的环境”的重复组合。
- 日常章节统一显示“日常怎么养”；具体内容卡仍显示“喂养方法”，不改变任何事实、证据或数据契约。
- 公开文案、结构、响应式、品系卡片、lint、build 和 diff-check 已通过。
- 目标仍在进行中：需要三档视觉复核；Critic 可读正文、Figma Canonical、远端推送和索引发布仍未完成。三个用户未提交文件保持原样。

- 已补充重复章节标题回归门禁，提交 `28014f9e`；后续视觉复核应以“单一章节标题 + 具体内容卡标题”为标准。
- 内置浏览器复核宝莲灯普通页面，确认图片回退、已确认环境/生活习性和缺失章节收拢正常；证据 `EVD-20260914-167`，不替代三档自动化截图。
- 内置浏览器复核黄金米虾和孔雀鱼：品系继承、参数差异、缺图回退和缺失正文边界正常；证据 `EVD-20260914-168`，不替代三档自动化截图。
- 内置浏览器接口不支持 `setViewportSize`，因此当前窗口不能作为精确 390px 证据；记录 `EVD-20260914-169`。
- 远端网络已恢复，分支成功推送并包含 `d3596494`；GitHub Actions 状态读取仍受 API 限流/CLI 连接问题影响，尚未计为浏览器通过。证据 `EVD-20260914-170`。
...
- 同一 Critic 线程已完成对最新版本的复验请求，但最新 turn 返回空 `items`、无可读正文（`EVD-20260914-148`）。不要创建重复线程或把旧报告当作当前复验通过；保持该门禁阻塞。
- 已根据旧 Critic 可读报告中的可执行问题强化 CI，提交 `884067ce`：44px 不再四舍五入，极火虾 FAQ 不存在时强制失败，并验证答案可见和焦点保持。静态检查通过，等待该提交的新 Actions 运行。
