...
import { loadReviewedCompatibilityAuthority } from '../compatibility-authority';
...
contentRouter.get('/compatibility-bootstrap', asyncRoute(async (request, response) => {
  const authority = await loadReviewedCompatibilityAuthority(getPublicSupabase());
  response.setHeader('Cache-Control', 'public, max-age=0, s-maxage=60, stale-while-revalidate=300');
  return sendData(request, response, authority);
}));
