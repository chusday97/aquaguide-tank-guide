...
assert.match(appSource, /currentWorkflowStage[\s\S]*dataReview\?\.pending[\s\S]*ready_for_review[\s\S]*publish_ready/, 'Global workflow priority stage must derive directly from real queue counts.');
assert.doesNotMatch(appSource, /workflow-progress-summary[\s\S]{0,220}<strong>/, 'Publish Flow summary must stay a section label instead of repeating a second n/4 progress indicator.');
assert.match(appSource, /ready_for_review \|\| 0\) > 0 \? <em>/, 'Zero-value global workflow counts must not render empty-noise badges.');
