...
assert.match(duplicateComparisonSource, /duplicate-keep-choice[\s\S]*type="radio"/, 'Choosing the canonical page must use a radio selection instead of another action button.');
assert.doesNotMatch(duplicateComparisonSource, /duplicate-keep-action/, 'Duplicate keep-page selection must not regress into a button wall.');
...
assert.match(appSource, /ready_for_review > 0 \? \([\s\S]*批量内容审核/, 'Zero-count bulk editorial review must not render as an actionable utility entry.');
...
assert.match(reviewSource, /review-evidence-section[\s\S]*review-decision-command/, 'Data Review must place evidence before the human conclusion.');
...
assert.match(uiFoundationSource, /review-evidence-section\.review-evidence-first[\s\S]*review-decision-command[\s\S]*position:\s*relative/, 'Data Review must keep evidence first and avoid a sticky decision block that competes with the evidence.');
...
assert.match(appSource, /workflow-stage-card needs-human-decision[\s\S]*数据复核[\s\S]*需人工确认/, 'Data Review stage must explicitly identify human-confirmation work.');
assert.match(uiFoundationSource, /needs-human-decision\.is-current[\s\S]*--state-warning/, 'Human-decision workflow stages must use the Amber decision token rather than navigation or publish colors.');
assert.match(uiFoundationSource, /tone-issue[\s\S]*--state-warning[\s\S]*review-confirm-action[\s\S]*--state-warning/, 'Data issue navigation and final confirmation must share the Amber decision grammar.');
...
assert.match(liveFrontendPreviewSource, /定位字段|Locate field/, 'Preview locator must use a clear utility action label.');
...
assert.match(appSource, /editor-secondary-seo-disclosure[\s\S]*focusKeyword[\s\S]*indexStrategy/, 'Focus keyword and indexing controls must stay behind the unified secondary SEO disclosure');
assert.match(appSource, /<textarea rows=\"4\" value=\{form\.intro\}/, 'Variant intro must default to a compact four-row editing surface');
assert.match(appSource, /secondary-seo-section policy-settings[\s\S]*focusKeyword[\s\S]*canonicalCatalogKey/, 'Low-frequency keyword/index/canonical controls must stay inside the secondary SEO policy section');
assert.match(appSource, /secondarySeoHasProblem[\s\S]*Boolean\(indexBlockReason\)[\s\S]*setSecondarySeoOpen\(true\)/, 'Secondary SEO must automatically surface active search/index/canonical blockers');
assert.match(appSource, /section-inline-reference-action[\s\S]*templateReferenceOpen[\s\S]*inline-template-reference/, 'Inherited Base content must stay on-demand inside the current task context.');
assert.doesNotMatch(appSource, /inherited-content-disclosure secondary-reference/, 'Template reference must not regress into a separate persistent disclosure row.');
assert.match(uiFoundationSource, /section-inline-reference-action[\s\S]*inline-template-reference/, 'UI Foundation must own the inline template-reference hierarchy.');
...
assert.match(bulkImportSource, /p_group_keys/, 'CSV bulk import must persist the complete Base-group scope for its import batch.');
assert.match(bulkImportSource, /p_batch_filename/, 'CSV bulk import must persist the uploaded filename on its import batch.');
assert.match(repoStoreSource, /import_batches/, 'Repo authority must persist import batches alongside Draft/review state.');
assert.match(repoStoreSource, /batch_id:\s*`batch-/, 'Repo authority must generate a durable server-side import batch id.');
assert.match(bulkEditorialSource, /p_batch_id/, 'Bulk editorial review must bind current-batch mutations to the persisted batch id.');
assert.match(bulkEditorialSource, /Latest import batch|最近导入批次/, 'Bulk editorial review must expose the latest import batch as the default safe scope.');
assert.match(bulkEditorialSource, /Select eligible in batch|全选本批可执行/, 'Bulk selection must default to the active batch rather than all pending content.');
assert.match(repoStoreSource, /outside import batch/, 'Repo review authority must reject resources outside a batch-bound review request.');
assert.match(repoStoreSource, /Staging allowlist must exactly match the selected import batch/, 'Staging publish must fail closed when the allowlist differs from the batch scope.');
assert.match(repoPublishApiSource, /batchId/, 'Staging API must pass the persisted batch id into server-side allowlist validation.');
assert.match(repoBackendClientSource, /JSON\.stringify\(\{ catalogKeys, groupKeys, batchId \}\)/, 'Browser staging client must send batch id with a batch-bound publish.');
assert.match(appSource, /from\('import_batches'\)/, 'Admin startup must reload persisted import batches after refresh/login.');
assert.match(bulkEditorialSource, /publishRecentBatch/, 'Bulk editorial review must provide an explicit import-batch Staging publish path.');
assert.match(bulkEditorialSource, /batchId:\s*recentImportScope\.batchId/, 'Batch Staging publish must carry the persisted batch id to server validation.');
assert.match(bulkEditorialSource, /scopeFromImportBatch/, 'Persisted Repo import batches must outrank browser-local scope recovery.');
assert.match(activityCenterSource, /metadata\?\.batch_id/, 'Activity Center must surface batch provenance for import/review/release actions.');
