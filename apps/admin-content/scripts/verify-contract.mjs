...
assert.match(bulkEditorialSource, /Current import batch|当前导入批次/, 'Bulk editorial review must expose the active import batch to the operator.');
assert.match(bulkEditorialSource, /Select current batch|全选当前批次/, 'Bulk selection must default to the active batch rather than all pending content.');
...
assert.match(appSource, /publishImportBatchToStaging/, 'Admin must provide an explicit import-batch Staging publish path.');
