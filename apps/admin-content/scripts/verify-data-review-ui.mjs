...
const runViewport = async (label, viewport) => {
  ...
  assert.equal(await page.locator('.topbar .activity-trigger').count(), 0, 'Activity history must not consume its own top-level button.');
  ...
  let operationsMenu = operationsDrawer.getByTestId('operations-tool-menu');
  assert.match(await operationsMenu.innerText(), /操作记录[\s\S]*批量审核重复记录[\s\S]*SEO 模板导入[\s\S]*任务队列/);
  await operationsMenu.getByTestId('operations-activity-entry').click();
  const activityCenter = page.locator('.activity-center');
  await activityCenter.waitFor();
  assert.match(await activityCenter.innerText(), /操作中心[\s\S]*只读界面演示不读取私有操作记录/);
  await page.getByRole('button', { name: '关闭操作中心', exact: true }).click();
  await activityCenter.waitFor({ state: 'detached' });
  await operationsTrigger.click();
  await operationsDrawer.waitFor();
  operationsMenu = operationsDrawer.getByTestId('operations-tool-menu');
  ...
  assert.equal(await page.locator('.topbar button:visible').count(), 4, 'Mobile topbar must expose only Operations, two language choices and sign out.');
