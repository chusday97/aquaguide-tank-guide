...
function ReviewDecision({ issueKey, issueType, group, set, row, schemaReady, readOnly, onSaved }) {
  ...
  return (
    <div className="review-decision-box">
      <label>人工结论
        <select value={decision} onChange={(event) => setDecision(event.target.value)}>
          ...
          {issueType === 'category_conflict' ? <>
            <option value="accepted_as_is">分类差异为预期，可继续 SEO</option>
            <option value="source_correction_required">源数据需要修正，继续阻止</option>
          </> : <>
            <option value="distinct_records">确认是不同记录</option>
            <option value="duplicate_records">确认重复，指定 canonical</option>
          </>}
        </select>
      </label>
      ...
      <label>审核备注
        <textarea rows="2" value={notes} onChange={(event) => setNotes(event.target.value)} placeholder="记录判断依据；不改写 Product Truth。" />
      </label>
      <div className="review-decision-footer">
        ...
        <button type="button" className="secondary-button compact" onClick={save} disabled={saving || readOnly}>{saving ? '保存中…' : '保存复核结论'}</button>
      </div>
    </div>
  );
}
    ...
    <section className="data-review-panel">
      <div className="data-review-header">
        <div>
          ...
          <h2>源数据复核</h2>
          <p>{group.base_scientific_name} 的问题需要人工结论；结论只影响 SEO 发布资格，不改 Product Truth。</p>
        </div>
      ...
      {group.category_conflict ? (
        <div className="review-issue-card">
          <div className="review-issue-title"><strong>分类冲突</strong><span>{group.categories.join(' ↔ ')}</span></div>
          <p>同一 Base Species 位于多个产品分类。只有人工确认“分类差异为预期”才能解除 SEO 阻止。</p>
      ...
      {group.duplicate_sets?.map((set) => (
        <div className="review-issue-card" key={set.duplicate_set_key}>
          <div className="review-issue-title"><strong>疑似完全重复</strong><span>{set.member_ids.length} 条记录</span></div>
